...
const TableView: <RecordType extends object = any>(
  ...
  if (props.dataSource) {
    console.error(
      'Please use the api to set the data of table-render. For specific usage, please refer to: https://form-render.github.io/table-render/guide/demo#%E5%9F%BA%E6%9C%AC-demo'
    );
  }
  ...
  const getProColumns = (columns: any[]) => {
    ...
    return columns.map((item) => {
      ...
      // The user-defined render in columns will override the default render of tr
      if (result.render) {
        return result;
      }
  ...
  // Apply columnsSetting to columns
  ...
  // Need to judge, otherwise some attributes of the table will be initialized and rendered abnormally
  if (columns?.length === 0) {
    return null;
  }
