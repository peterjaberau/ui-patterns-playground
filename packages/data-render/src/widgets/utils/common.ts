...
/**
 * @param path // The path of the corresponding data, for example: 'abc'
 * @param defaultValue // default value
 * @param data // data
 * @param addons // Renderer method set, access getSourceData
 * @returns value
 */
export const getValueFromKey = (props: {
  ...
  let result: any = null;
  let negation: any = null; // Negation flag!
  let dataPath = path;
  ...
  // With source flag, it means getting data from the top level
  ...
  // Negate according to negation and return the result
  if (negation === '!') {
    return !result;
  }
...
export const transformData = (value: any, format: any, parentData?: any, addons?: any) => {
  ...
  const getValue = (value: any, item: any) => {
    ...
    if (result === 'Invalid Date') {
      result = '';
      console.warn('Date conversion error');
    }

    if (result === 'NaN%') {
      result = '';
      console.warn('Percentage conversion error');
    }
...
// Determine whether it is a subprotocol
