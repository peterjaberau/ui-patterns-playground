...
import type { MarketplaceCollection, PluginsSort, SearchParams, SearchParamsFromCollection } from "./types";
import { DEFAULT_SORT } from "./constants";
import { useMarketplaceCollectionsAndPlugins, useMarketplaceContainerScroll, useMarketplacePlugins } from "./hooks";
import { getMarketplaceListCondition, getMarketplaceListFilterType } from "./utils";
...
export const MarketplaceContextProvider = ({
  ...
  const data: any = { plugins: [] }; // Mock data for testing
  const isSuccess = true;
  const exclude = useMemo(() => {
    if (shouldExclude) return data?.plugins.map((plugin: any) => plugin.plugin_id);
  ...
  const handleQueryPlugins = useCallback(
    (debounced?: boolean) => {
      if (debounced) {
        queryPluginsWithDebounced({
          query: searchPluginTextRef.current,
          category:
            activePluginTypeRef.current === PLUGIN_TYPE_SEARCH_MAP.all ? undefined : activePluginTypeRef.current,
  ...
  const handleSearchPluginTextChange = useCallback(
    (text: string) => {
  ...
  const handleFilterPluginTagsChange = useCallback(
    (tags: string[]) => {
