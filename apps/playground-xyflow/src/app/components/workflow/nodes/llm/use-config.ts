...
const useConfig = (id: string, payload: LLMNodeType) => {
    ...
    (oldName: string, newName: string) => {
      const newInputs = produce(inputRef.current, (draft) => {
        if (isChatModel) {
          ...
          if ((draft.prompt_template as PromptItem).edition_type !== EditionType.jinja2) return;
  ...
  const filterJinjia2InputVar = useCallback((varPayload: Var) => {
    return [VarType.number, VarType.string, VarType.secret, VarType.arrayString, VarType.arrayNumber].includes(
      varPayload.type
    );
  }, []);
  ...
  const varInputs = (() => {
    ...
    if (isShowVars) return [...vars, ...toVarInputs(inputs.prompt_config?.jinja2_variables || [])];
