...
import { PluginType, type StrategyPluginDetail } from "@/types/components/plugins/types";
...
export const AgentStrategySelector = memo((props: AgentStrategySelectorProps) => {
  ...
  const filteredTools = useMemo(() => {
    if (!list) return [];
    return list.filter((tool: any) => tool.name.toLowerCase().includes(query.toLowerCase()));
  }, [query, list]);
  ...
  const icon = list?.find((coll: any) => coll.tools?.find((tool: any) => tool.name === value?.agent_strategy_name))
        ...
        >
          ...
          <p
            className={classNames(
              value ? "text-components-input-text-filled" : "text-components-input-text-placeholder",
              "text-xs px-1"
            )}
          >
          ...
          <div className="ml-auto flex items-center gap-1">
            ...
            {showSwitchVersion && (
              <SwitchPluginVersion
                uniqueIdentifier={value.plugin_unique_identifier}
                tooltip={
                  <ToolTipContent title={t("workflow.nodes.agent.unsupportedStrategy")}>
      ...
      <PortalToFollowElemContent className="z-10">
        <div className="w-[388px] overflow-hidden rounded-md border-[0.5px] border-components-panel-border bg-components-panel-bg-blur shadow">
          <header className="flex gap-1 p-2">
            <SearchInput
              placeholder={t("workflow.nodes.agent.strategy.searchPlaceholder")}
              value={query}
              onChange={setQuery}
              className={"w-full"}
            />
          ...
          <main
            className="relative flex w-full flex-col overflow-hidden md:max-h-[300px] xl:max-h-[400px] 2xl:max-h-[564px]"
            ref={wrapElemRef}
          >
            <Tools
              tools={filteredTools}
            ...
            {/*   list={notInstalledPlugins as any} */}
            {/*   ref={pluginRef} */}
            {/*   searchText={query} */}
            {/*   tags={[]} */}
            {/*   disableMaxWidth */}
