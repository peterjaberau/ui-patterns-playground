...
const ConstantField: FC<Props> = ({ schema = {} as CredentialFormSchema, readonly, value, onChange }) => {
    ...
    <>
      {schema.type === "select" && (
        <SimpleSelect
          ...
          disabled={readonly}
          defaultValue={value}
          items={(schema as CredentialFormSchemaSelect).options.map((option) => ({
            value: option.value,
            name: option.label.en_US,
          }))}
          onSelect={(item) => handleSelectChange(item.value)}
          placeholder={placeholder?.en_US}
        />
      )}
      {schema.type === "number-input" && (
        <input
          type="number"
