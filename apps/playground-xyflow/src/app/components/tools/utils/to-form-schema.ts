import type { ToolCredential, ToolParameter } from "@/types/components/tools/types";
...
export const addDefaultValue = (value: Record<string, any>, formSchemas: { variable: string; default?: any }[]) => {
  ...
  formSchemas.forEach((formSchema) => {
    ...
    if (
      formSchema.default !== undefined &&
      (value === undefined || itemValue === null || itemValue === "" || itemValue === undefined)
    )
...
export const generateFormValue = (
  ...
  formSchemas: { variable: string; default?: any }[],
  isReasoning = false
) => {
