...
const getKey = (
  ...
  if (!pageIndex || previousPageData.has_more) {
    const params: any = {
      url: "apps",
      params: { page: pageIndex + 1, limit: 30, name: keywords, is_created_by_me: isCreatedByMe },
    };
...
const Apps = () => {
  ...
  const {
    query: { tagIDs = [], keywords = "", isCreatedByMe: queryIsCreatedByMe = false },
    setQuery,
  } = useAppsQueryState();
  ...
  const { data, isLoading, setSize, mutate } = useSWRInfinite(
    (pageIndex: number, previousPageData: AppListResponse) =>
      getKey(pageIndex, previousPageData, activeTab, isCreatedByMe, tagIDs, searchKeywords),
    fetchAppList,
    { revalidateFirstPage: true }
  );

  const anchorRef = useRef<HTMLDivElement | any>(null);
  const options = [
    { value: "all", text: "All", icon: <RiApps2Line className="mr-1 h-[14px] w-[14px]" /> },
    { value: "chat", text: "CHATBOT", icon: <RiMessage3Line className="mr-1 h-[14px] w-[14px]" /> },
    { value: "agent-chat", text: "AGENT", icon: <RiRobot3Line className="mr-1 h-[14px] w-[14px]" /> },
    { value: "completion", text: "COMPLETION", icon: <RiFile4Line className="mr-1 h-[14px] w-[14px]" /> },
    {
      value: "advanced-chat",
      text: "ADVANCED",
      icon: <RiMessage3Line className="mr-1 h-[14px] w-[14px]" />,
    },
    { value: "workflow", text: "WORKFLOW", icon: <RiExchange2Line className="mr-1 h-[14px] w-[14px]" /> },
  ];
  ...
  useEffect(() => {
    ...
    if (anchorRef.current) {
      observer = new IntersectionObserver(
        (entries) => {
          if (entries[0].isIntersecting && !isLoading && hasMore) setSize((size: number) => size + 1);
        },
        { rootMargin: "100px" }
      );
  ...
  return (
    <>
      ...
      <div className="absolute bottom-0 left-0 right-0 top-0 flex items-center justify-center bg-gradient-to-t from-background-body to-transparent">
        <span className="system-md-medium text-text-tertiary">NO APPS FOUND</span>
      </div>
    </>
  );
}
