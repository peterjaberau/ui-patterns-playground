import { InputVarType } from "@/types/components/workflow/types";
...
if (process.env.NEXT_PUBLIC_API_PREFIX && process.env.NEXT_PUBLIC_PUBLIC_API_PREFIX) {
  ...
  publicApiPrefix = "http://localhost:4055/api"; // avoid browser private mode api cross origin
  marketplaceApiPrefix = "http://localhost:4056/api";
}
...
const EDITION =
  process.env.NEXT_PUBLIC_EDITION || globalThis.document?.body?.getAttribute("data-public-edition") || "SELF_HOSTED";
...
if (process.env.NEXT_PUBLIC_TEXT_GENERATION_TIMEOUT_MS && process.env.NEXT_PUBLIC_TEXT_GENERATION_TIMEOUT_MS !== "")
  ...
  globalThis.document?.body?.getAttribute("data-public-text-generation-timeout-ms") &&
  globalThis.document.body.getAttribute("data-public-text-generation-timeout-ms") !== ""
)
  textGenerationTimeoutMs = Number.parseInt(
    globalThis.document.body.getAttribute("data-public-text-generation-timeout-ms") as string
  );
...
if (process.env.NEXT_PUBLIC_LOOP_NODE_MAX_COUNT && process.env.NEXT_PUBLIC_LOOP_NODE_MAX_COUNT !== "")
  ...
  loopNodeMaxCount = Number.parseInt(
    globalThis.document.body.getAttribute("data-public-loop-node-max-count") as string
  );
