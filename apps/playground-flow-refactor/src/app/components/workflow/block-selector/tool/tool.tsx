...
import type { Tool as ToolType } from "@/types/components/tools/types";
...
const Tool: FC<Props> = ({
  ...
  const getIsDisabled = (tool: ToolType) => {
    ...
    return selectedTools.some(
      (selectedTool) => selectedTool.provider_name === payload.name && selectedTool.tool_name === tool.name
    );
  };
  useEffect(() => {
  ...
  const groupName = useMemo(() => {
    ...
    if (payload.type === CollectionType.custom) return t("workflow.tabs.customTool");

    if (payload.type === CollectionType.workflow) return t("workflow.tabs.workflowTool");

    return "";
  }, [payload.author, payload.type, t]);

  return (
    <div key={payload.id} className={cn("mb-1 last-of-type:mb-0", isShowLetterIndex && "mr-6")}>
      <div className={cn(className)}>
        ...
        >
          ...
          <div className="flex items-center">
            {isFlatView && <div className="system-xs-regular text-text-tertiary">{groupName}</div>}
            {hasAction && (
