...
import type { CredentialFormSchema } from "@/types/account/model-provider/declarations";
import { FormTypeEnum } from "@/types/account/model-provider/declarations";
...
const InputVarList: FC<Props> = ({
  ...
  const paramType = (type: string) => {
    ...
    else if (type === FormTypeEnum.file || type === FormTypeEnum.files) return "Files";
    else if (type === FormTypeEnum.appSelector) return "AppSelector";
    else if (type === FormTypeEnum.modelSelector) return "ModelSelector";
    else if (type === FormTypeEnum.toolSelector) return "ToolSelector";
    else return "String";
  };
  ...
  const handleAppChange = useCallback(
    (variable: string) => {
      return (app: { app_id: string; inputs: Record<string, any>; files?: any[] }) => {
        const newValue = produce(value, (draft: ToolVarInputs) => {
  ...
  return (
    <div className="space-y-3">
      {schema.map((schema, index) => {
        const { variable, label, type, required, tooltip, scope } = schema;
        ...
        return (
          <div key={variable} className="space-y-1">
            <div className="flex h-[18px] items-center space-x-2">
              <span className="code-sm-semibold text-text-secondary">{label.en_US}</span>
              <span className="system-xs-regular text-text-tertiary">{paramType(type)}</span>
              {required && (
                <span className="system-xs-regular text-util-colors-orange-dark-orange-dark-600">Required</span>
              )}
            </div>
            {isString && (
              <Input
                className={cn(
                  inputsIsFocus[variable] ? "border-gray-300 bg-gray-50 shadow-xs" : "border-gray-100 bg-gray-100",
            ...
            {isFile && (
              <VarReferencePicker
                ...
                filterVar={(varPayload: Var) =>
                  varPayload.type === VarType.file || varPayload.type === VarType.arrayFile
                }
              />
            )}
            {/* {isAppSelector && ( */}
            ...
            {/*   /> */}
            {/* )} */}
            {tooltip && <div className="body-xs-regular text-text-tertiary">{tooltip.en_US}</div>}
          </div>
        );
      })}
    </div>
