...
const AgentNode: FC<NodeProps<AgentNodeType>> = (props) => {
  ...
  const models = useMemo(() => {
    ...
    const models =
      currentStrategy?.parameters
        .filter((param: any) => param.type === FormTypeEnum.modelSelector)
  ...
  const tools = useMemo(() => {
    ...
    currentStrategy?.parameters.forEach((param: any) => {
    ...
    <div className="mb-1 space-y-1 px-3 py-1">
      {inputs.agent_strategy_name ? (
        <SettingItem
          ...
          status={currentStrategyStatus && !currentStrategyStatus.isExistInPlugin ? "error" : undefined}
          tooltip={
      ...
      {models.length > 0 && (
        <Group label={<GroupLabel className="mt-1">{t("workflow.nodes.agent.model")}</GroupLabel>}>
      ...
      {tools.length > 0 && (
        <Group label={<GroupLabel className="mt-1">{t("workflow.nodes.agent.toolbox")}</GroupLabel>}>
          <div className="grid grid-cols-10 gap-0.5">
            {tools.map((tool) => (
              <ToolIcon {...tool} key={Math.random()} />
            ))}
          </div>
        </Group>
      )}
