...
const DatasetsDetailProvider: FC<DatasetsDetailProviderProps> = ({ nodes, children }) => {
  ...
  const updateDatasetsDetail = useCallback(async (datasetIds: string[]) => {
    ...
    const datasetsDetail: any = [];
    if (datasetsDetail && datasetsDetail.length > 0) storeRef.current!.getState().updateDatasetsDetail(datasetsDetail);
  }, []);
  ...
  return <DatasetsDetailContext.Provider value={storeRef.current!}>{children}</DatasetsDetailContext.Provider>;
};
