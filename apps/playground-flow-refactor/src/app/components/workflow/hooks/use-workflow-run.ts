...
export const useWorkflowRun = () => {
  ...
  const handleRun = useCallback(
    async (params: any, callback?: IOtherOptions) => {
      ...
      let url = "";
      if (appDetail?.mode === "advanced-chat") url = `/apps/${appDetail.id}/advanced-chat/workflows/draft/run`;

      if (appDetail?.mode === "workflow") url = `/apps/${appDetail.id}/workflows/draft/run`;
  ...
  const handleRestoreFromPublishedWorkflow = useCallback(
    (publishedWorkflow: VersionHistory) => {
      ...
      const mappedFeatures = {
        opening: {
          enabled:
            !!publishedWorkflow.features.opening_statement || !!publishedWorkflow.features.suggested_questions.length,
