...
import {
  ...
  fetchWorkflowDraft,
  // syncWorkflowDraft,
} from "@/service/.mock-api/workflow";
import type { FetchWorkflowDraftResponse } from "@/types/workflow";
import {
  fetchAllBuiltInTools,
  fetchAllCustomTools,
  // fetchAllWorkflowTools,
} from "@/service/.mock-api/tools";
import I18n from "@/context/i18n";
import { CollectionType } from "@/types/components/tools/types";
import { CUSTOM_ITERATION_START_NODE } from "@/app/components/workflow/nodes/iteration-start/constants";
import { CUSTOM_LOOP_START_NODE } from "@/app/components/workflow/nodes/loop-start/constants";
import { useWorkflowConfig } from "@/service/.mock-api/use-workflow";
...
export const useWorkflow = () => {
  ...
  const formatTimeFromNow: any = useCallback(
    (time: number) => {
      return (
        dayjs(time)
...
export const useFetchToolsData = () => {
  ...
  const handleFetchAllTools = useCallback(
    async (type: string) => {
      if (type === "builtin") {
        // const buildInTools: any = await fetchAllBuiltInTools();
        const buildInTools: any = fetchAllBuiltInTools();
      ...
      if (type === "custom") {
        ...
        const customTools: any = fetchAllCustomTools();
...
export const useWorkflowInit = () => {
  ...
  const handleGetInitialWorkflowData = useCallback(async () => {
    try {
      const res: any = await fetchWorkflowDraft(`/apps/${appDetail.id}/workflows/draft`);
      setData(res);
      workflowStore.setState({
        envSecrets: (res.environment_variables || [])
          .filter((env: any) => env.value_type === "secret")
          .reduce((acc: any, env: any) => {
            acc[env.id] = env.value;
            return acc;
          }, {} as Record<string, string>),
        environmentVariables:
          res.environment_variables?.map((env: any) =>
  ...
  const handleFetchPreloadData = useCallback(async () => {
    try {
      ...
      const publishedWorkflow: any = await fetchPublishedWorkflow(`/apps/${appDetail?.id}/workflows/publish`);
      workflowStore.setState({
        nodesDefaultConfigs: nodesDefaultConfigsData.reduce((acc: any, block: any) => {
          if (!acc[block.type as any]) acc[block.type] = { ...block.config };
          return acc;
