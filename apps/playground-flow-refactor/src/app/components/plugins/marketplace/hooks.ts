...
import type { CollectionsAndPluginsSearchParams, MarketplaceCollection, PluginsSearchParams } from "./types";
import { getFormattedPlugin, getMarketplaceCollectionsAndPlugins } from "./utils";
...
export const useMarketplacePlugins = () => {
  // const { data, mutateAsync, reset, isPending } = useMutationPluginsFromMarketplace();
  const data: any = {};
  ...
  //       if (currentPage > 1) {
  //         setPrevPlugins((prevPlugins) => [
  //           ...(prevPlugins || []),
  //           ...resPlugins.map((plugin) => {
...
export const useMixedTranslation = (localeFromOuter?: string) => {
  ...
  if (localeFromOuter) t = i18n.getFixedT(localeFromOuter);
...
export const useMarketplaceContainerScroll = (callback: () => void, scrollContainerId = "marketplace-container") => {
  ...
  const handleScroll = useCallback(
    (e: Event) => {
      ...
      if (scrollTop + clientHeight >= scrollHeight - 5 && scrollTop > 0) callback();
    },
    [callback]
  );
...
export const useSearchBoxAutoAnimate = (searchBoxAutoAnimate?: boolean) => {
  ...
  const handleSearchBoxCanAnimateChange = useCallback(() => {
    if (!searchBoxAutoAnimate) {
      ...
      if (clientWidth < 1400) setSearchBoxCanAnimate(false);
      else setSearchBoxCanAnimate(true);
    }
