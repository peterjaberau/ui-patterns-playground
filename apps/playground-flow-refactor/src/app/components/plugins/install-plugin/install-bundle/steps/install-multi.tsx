...
const InstallByDSLList: FC<Props> = ({
  ...
  const handleGitHubPluginFetched = useCallback(
    (index: number) => {
      return (p: Plugin) => {
  ...
  const handleGitHubPluginFetchError = useCallback(
    (index: number) => {
      return () => {
  ...
  const marketPlaceInDSLIndex = useMemo(() => {
    ...
    allPlugins.forEach((d, index) => {
      if (d.type === "marketplace") res.push(index);
    });
    return res;
  }, [allPlugins]);
