...
import { ComponentProps, Fragment, PropsWithChildren, ReactNode, useEffect, useRef, useState } from 'react';
  ...
  return (
    <>
      ...
      {Array.from(clones.entries()).map(([id, clone]) => (
        <Fragment key={id}>{createPortal(content ?? children, clone.children[0])}</Fragment>
      ))}
    </>
  );
}
