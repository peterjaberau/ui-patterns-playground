...
import { useProductTags, useProductTypes, useSalesChannels } from '../_mock_';

const excludeableFields = ['sales_channel_id', 'collections', 'categories', 'product_types', 'product_tags'] as const;
...
export const useProductTableFilters = (exclude?: (typeof excludeableFields)[number][]) => {
  ...
  if (product_types && !isProductTypeExcluded) {
    const typeFilter: Filter = {
      ...
      type: 'select',
      multiple: true,
      options: product_types.map((t: any) => ({
        label: t.value,
        value: t.id,
      })),
    };
  ...
  if (product_tags && !isProductTagExcluded) {
    const tagFilter: Filter = {
      ...
      type: 'select',
      multiple: true,
      options: product_tags.map((t: any) => ({
        label: t.value,
        value: t.id,
      })),
    };
