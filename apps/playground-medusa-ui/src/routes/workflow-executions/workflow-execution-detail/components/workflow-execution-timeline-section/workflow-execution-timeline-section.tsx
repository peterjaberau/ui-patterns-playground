...
type WorkflowExecutionTimelineSectionProps = {
  execution: HttpTypes.AdminWorkflowExecutionResponse['workflow_execution'] | any;
};

export const WorkflowExecutionTimelineSection = ({ execution }: WorkflowExecutionTimelineSectionProps) => {
  return (
...
const getNextCluster = (clusters: Record<number, HttpTypes.AdminWorkflowExecutionStep[]>, depth: number) => {
...
const Canvas = ({ execution }: { execution: HttpTypes.AdminWorkflowExecutionResponse['workflow_execution'] }) => {
...
const Arrow = ({ depth }: { depth: number }) => {
  ...
  const inbetween = Array.from({ length: depth - 2 }).map((_, index) => <MiddleArrow key={index} />);
