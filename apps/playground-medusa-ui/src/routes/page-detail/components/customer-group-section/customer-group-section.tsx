...
import { useCustomerGroups } from '@/hooks/table/_mock_';
...
export const CustomerGroupSection = ({ customer }: CustomerGroupSectionProps) => {
  ...
  const handleRemove = async () => {
    ...
    const res = await prompt({
      ...
      description: `Are you sure you want to remove customer from following customer groups: ${customer_groups
        ?.filter((g: any) => customerGroupIds.includes(g.id))
        .map((g: any) => g.name)
        .join(',')}?`,
  ...
  return (
    <Container className="divide-y p-0">
      <div className="flex items-center justify-between px-6 py-4">
        ...
        <Link href={`/customers/${customer.id}/add-customer-groups`}>
...
const CustomerGroupRowActions = ({
  ...
  const onRemove = async () => {
    const res = await prompt({
      title: t('general.areYouSure'),
      description: `Are you sure you want to remove customer from ${group.name} customer group?`,
      confirmText: t('actions.remove'),
      cancelText: t('actions.cancel'),
    });
...
const useColumns = (customerId: string) => {
  ...
  return useMemo(
    () => [
      ...
      columnHelper.display({
        id: 'actions',
        cell: ({ row }) => <CustomerGroupRowActions group={row.original} customerId={customerId} />,
      }),
    ],
    [columns, customerId],
  );
