...
import { useCancelOrderTransfer, useCustomer, useOrderChanges, useOrderLineItems } from '../../../../../hooks/api';
...
const useActivityItems = (order: AdminOrder): Activity[] => {
  ...
  return useMemo(() => {
    ...
    for (const claim of claims) {
      ...
      items.push({
        title: t(claim.canceled_at ? 'orders.activity.events.claim.canceled' : 'orders.activity.events.claim.created', {
...
const ReturnBody = ({
  ...
  const { mutateAsync: cancelReturnRequest } = useCancelReturn(orderReturn.id, orderReturn.order_id);
...
const ClaimBody = ({ claim, claimReturn }: { claim: AdminClaim; claimReturn?: AdminReturn }) => {
  ...
  const outboundItems = (claim.additional_items || []).reduce((acc, item) => (acc + item.quantity) as number, 0);
...
const ExchangeBody = ({ exchange, exchangeReturn }: { exchange: AdminExchange; exchangeReturn?: AdminReturn }) => {
  const prompt = usePrompt();
  const { t } = useTranslation();
  ...
  const outboundItems = (exchange.additional_items || []).reduce((acc, item) => (acc + item.quantity) as number, 0);
