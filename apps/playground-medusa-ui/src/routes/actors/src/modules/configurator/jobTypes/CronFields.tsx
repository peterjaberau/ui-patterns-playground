...
export const CronFields = ({ className = '' }: CronFieldsProps) => {
  ...
  const cron = useSelector(globalServices.workspaceService, cronValueSelector);
  ...
  const setCronValidity = (isValid: boolean) =>
  ...
  return (
    <div className={`${className} flex w-full max-w-xs flex-col`}>
      ...
      />
      <p className="mt-2 max-w-[12rem] text-xs italic">{friendlyCron}</p>
    </div>
  );
};
