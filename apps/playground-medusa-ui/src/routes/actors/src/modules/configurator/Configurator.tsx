...
export const Configurator = () => {
  ...
  const externalJobId = useSelector(globalServices.workspaceService, externalJobIdSelector);
  const gasLimit = useSelector(globalServices.workspaceService, gasLimitSelector);
  const maxTaskDuration = useSelector(globalServices.workspaceService, maxTaskDurationSelector);
  const forwardingAllowed = useSelector(globalServices.workspaceService, forwardingAllowedSelector);
  ...
  return (
    <div className="">
      <div className="mb-6 flex items-center justify-start gap-2">
        <h4 className="text-muted-foreground text-sm font-bold uppercase tracking-wider">Config</h4>
        <Tooltip className="text-muted-foreground text-sm">
          <p>
            Chainlink nodes support the execution of a number of job types. Each job type has some unique fields and
            pipeline variables.
          </p>
      ...
      <div className="grid max-w-xl grid-cols-2 gap-3">
        ...
        <div className="flex w-full max-w-sm flex-col">
          ...
          <Input
            ...
            value={externalJobId}
            onChange={(event) =>
              globalServices.workspaceService.send('SET_EXTERNAL_JOB_ID', { value: event.target.value })
            }
          />
        </div>
        ...
        <div className="flex w-full max-w-sm flex-col">
          ...
          <Input
            ...
            onChange={(event) =>
              globalServices.workspaceService.send('SET_MAX_TASK_DURATION', { value: event.target.value })
            }
          />
        </div>
        ...
        <div className="flex w-full max-w-sm flex-col">
          ...
          <Select
            ...
            onValueChange={(newValue) =>
              globalServices.workspaceService.send('SET_FORWARDING_ALLOWED', { value: newValue })
            }
          >
            <SelectTrigger className="w-[180px]">
              <SelectValue />
      ...
      <div className="mt-6 w-full min-w-max rounded-lg border p-2">
        <div className="mb-4 flex w-full max-w-xs flex-col">
          ...
          >
            ...
            <SelectContent>
              ...
              <SelectItem value="fluxmonitor" disabled>
                Flux Monitor
              </SelectItem>
              <SelectItem value="keeper" disabled>
                Keeper
              </SelectItem>
              <SelectItem value="offchainreporting" disabled>
                Off-chain Reporting
              </SelectItem>
        ...
        <TaskConfigTabs
          config={
            <div className="bg-background flex h-full w-full flex-col gap-2 p-4">
