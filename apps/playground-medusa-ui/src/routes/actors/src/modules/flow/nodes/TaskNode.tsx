...
import { XYCoords } from '../../workspace/types/nodeType';
...
export const TaskNode = ({ id, data, useDefaultHandles = true, children, type }: TaskNodeProps) => {
  ...
  const updateExistingConnections = () => {
    ...
    const incomingNodes = incomingNodeIds.map((nodeId: string) => getTaskNodeByCustomId(nodeId));
  ...
  const getNodePosition = (): XYCoords => {
    return reactFlowInstance ? reactFlowInstance.getNode(machine.id)?.position : { x: 0, y: 0 };
  };
  ...
  return (
    <div className="relative isolate overflow-visible">
      ...
      <div className="bg-popover ring-accent/60 relative flex w-[300px] cursor-default flex-col items-center justify-center rounded-lg p-1 shadow-lg ring">
        {(isPendingRun || isPendingSideEffect) && (
          <div className="absolute bottom-0 left-0 right-0 top-0 z-0 flex flex-col items-center justify-center overflow-hidden rounded-lg">
            <div className="bg-gradient-conic from-pending-light via-pending via-pending-dark via-pending to-pending-light absolute h-[2000px] w-[2000px] animate-spin"></div>
          </div>
        )}
        {(isRunning || isRunningSideEffect) && (
          <div className="absolute bottom-0 left-0 right-0 top-0 z-0 flex flex-col items-center justify-center overflow-hidden rounded-lg">
            <div className="bg-gradient-conic from-background to-pending absolute h-[2000px] w-[2000px] animate-spin"></div>
          </div>
        )}
        {isSuccess && (
          <div className="absolute bottom-0 left-0 right-0 top-0 z-0 flex flex-col items-center justify-center overflow-hidden rounded-lg">
        ...
        <div className="bg-background relative z-10 h-full w-full rounded-md p-3">
          ...
          {testMode && (
            <div className="transparent absolute inset-0 z-20">
              <div className="bg-background absolute inset-0 rounded-lg opacity-20" />
          ...
          <div className="custom-drag-handle absolute right-2 top-[6px] z-30 flex h-10 w-6 cursor-grab items-center justify-center">
          ...
          <div className="relative">
            <div className="flex flex-row items-center gap-2">
              <p className="text-primary text-xl font-bold">{data.type}</p>
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    <Squares2X2Icon className="group-hover:stroke-background h-4 w-4" />
            ...
            <div className="flex w-full max-w-xs flex-col">
              <div className="flex items-end justify-between">
                ...
                {customIdError && <span className="text-error label-text-alt mb-[2px] text-xs">Not unique</span>}
              </div>
              <Input
                id={taskIdFieldId}
                value={tempCustomId}
