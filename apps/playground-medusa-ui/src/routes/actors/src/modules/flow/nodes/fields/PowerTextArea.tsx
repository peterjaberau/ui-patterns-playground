...
const stripSpanTags = (input: string) => input.replaceAll(/(<([^>]+)>)/gi, '');
      ...
      <div className="relative">
        <div className="grid grid-cols-1 grid-rows-1">
          <ContentEditable
            ...
            onFocus={handleFocus}
            onPaste={handlePaste}
            className={`${textAreaClasses} col-span-full row-span-full h-full w-full pr-8`}
          />
          <ContentEditable
            html={value.rich || `<div class="text-muted-foreground">${placeholder}</div>`}
            onChange={() => {}}
            className={`${textAreaClasses} ${showRich ? '' : 'invisible'} pointer-events-none col-span-full row-span-full h-full pr-8`}
          />
        </div>
