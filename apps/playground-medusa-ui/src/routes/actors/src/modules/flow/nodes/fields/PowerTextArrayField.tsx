...
import { JOB_TYPE } from '../../../workspace/types/workspaceType';
...
const taskCustomIdsSelector = (state: any) =>
  state.context.nodes.tasks.map((task: any) => task.ref.state.context.customId);
...
const stripSpanTags = (input: string) => input.replaceAll(/<\/*span[^>]*>/gi, '');
...
export const PowerTextArrayField = ({
  ...
  const jobType: JOB_TYPE = useSelector(globalServices.workspaceService, jobTypeSelector);

  const tasks = useSelector(globalServices.workspaceService, taskCustomIdsSelector);
  ...
  const handleChange = (event: ContentEditableEvent) => {
    ...
    const asArrayVal = convertNewLinesToArrayValue(newVal);
    onChange(newVal.length > 0 ? asArrayVal : '', event.target.value);
  };
  ...
  const textAreaClasses =
    'h-10 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50';

  return (
    <div className={`${className} flex w-full max-w-xs flex-col`}>
      {/* @ts-ignore */}
      ...
      <div className="relative">
        <div className="grid grid-cols-1 grid-rows-1">
          <ContentEditable
            ...
            onFocus={handleFocus}
            onPaste={handlePaste}
            className={`${textAreaClasses} col-span-full row-span-full h-full overflow-auto whitespace-nowrap`}
          />
          <ContentEditable
            html={value.rich || `<div class="text-muted-foreground">${placeholder}</div>`}
            onChange={() => {}}
            className={`${textAreaClasses} ${showRich ? '' : 'invisible'} textarea textarea-bordered pointer-events-none col-span-full row-span-full h-full overflow-auto whitespace-nowrap pr-8`}
          />
        </div>
        <div className="absolute bottom-1 right-1">
          <Popover>
            <PopoverTrigger asChild>
              ...
              >
                <BoltIcon className="group-hover:stroke-background h-4 w-4" />
...
const convertArrayValueToNewlines = (value: string) => {
  return value
    ? value
        .slice(1, -1)
        .split(',')
        .map((val, index) => (index === 0 ? val : `<div>${val}</div>`))
        .join('')
