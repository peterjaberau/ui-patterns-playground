...
const Background = dynamic<BackgroundProps>(
  import('reactflow').then((mod) => mod.Background) as DynamicOptions<{}> | Loader<{}>,
  { ssr: false },
); // disable ssr
import {
  AiPromptNode,
...
export const Flow = ({ className }: FlowProps) => {
  ...
  const edgesFromMachine = useSelector(globalServices.workspaceService, edgesSelector);
  ...
  const isDefaultMode = useSelector(globalServices.workspaceService, defaultModeSelector);

  const isAiWandMode = useSelector(globalServices.workspaceService, aiWandModeSelector);
  ...
  const elements = [
    ...taskNodesFromMachine.map((node: any, index: number) =>
      taskNodeToFlowElement(
        node,
        index,
        taskNodesFromMachine.length,
        taskNodesFromMachine.length + aiNodesFromMachine.length,
      ),
    ),
    ...aiNodesFromMachine.map((node: any, index: number) =>
      aiNodeToFlowElement(node, index, aiNodesFromMachine.length),
    ),
  ];
  ...
  const getTaskNodeById = (nodeId: string) => taskNodesFromMachine.find((taskNode: any) => taskNode.ref.id === nodeId);
