...
export const workspaceMachineOptions: any = {
  actions: {
      ...
      if (error) {
        ...
        toast.custom((t) => (
          <div
            ...
            } bg-background ring-secondary pointer-events-auto flex w-full max-w-md rounded-lg shadow-lg ring-1`}
          >
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-2 text-sm">
                    <p className="">We imported as much as possible but there were some issues:</p>
            ...
            <div className="flex border-l border-gray-300">
              <button
                ...
                className="text-muted-foreground focus:ring-secondary flex w-full items-center justify-center rounded-none rounded-r-lg border border-transparent p-4 text-sm font-medium hover:text-white focus:outline-none focus:ring-1"
              >
                Close
              </button>
            </div>
    ...
    handleConnectionSuccessTaskNodeAddition: raise(({ context, event }: any) => {
      ...
      const initialCoords =
        'initialCoords' in event
          ? adjustNewSourceNodeHeightByTypeDefault(event.initialCoords, taskType, isForwardConnection)
          : { x: 0, y: 0 };
    ...
    handleConnectionSuccessAiPromptNodeAddition: raise(({ context, event }: any) => {
      ...
      const initialCoords =
        'initialCoords' in event
          ? adjustNewSourceNodeHeight(event.initialCoords, 200, isForwardConnection)
          : { x: 0, y: 0 };
    ...
    regenerateToml: assign(({ context, event }: any) => {
      ...
      gasLimit && lines.push({ value: `gasLimit = "${gasLimit}"` });

      maxTaskDuration && lines.push({ value: `maxTaskDuration = "${maxTaskDuration}"` });

      forwardingAllowed && lines.push({ value: `forwardingAllowed = "${forwardingAllowed}"` });
      ...
      switch (jobType) {
        ...
        case 'directrequest': {
          ...
          const { value: minContractPaymentLinkJuels, valid: minContractPaymentLinkJuelsValid } =
            context.jobTypeSpecific.directrequest.minContractPaymentLinkJuels;
          const { value: minIncomingConfirmations, valid: minIncomingConfirmationsValid } =
        ...
        switch (taskType) {
          case 'HTTP': {
            const processedRequestData = taskSpecific.requestData?.raw
              ? taskSpecific.requestData?.raw.replace(/\s/g, '').replace(/"/g, `\\\"`)
              : '';
          ...
          case 'JSONPARSE': {
            const processedData = taskSpecific.data?.raw
              ? taskSpecific.data?.raw.replace(/\s/g, '').replace(/"/g, `\\\"`)
              : '';
