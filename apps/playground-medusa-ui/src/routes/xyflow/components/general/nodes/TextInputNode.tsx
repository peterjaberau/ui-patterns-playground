...
export default memo(({ id }: any) => {
  ...
  const dimensions: any = useStore((s) => {
    ...
    if (!node || !node.measured.width || !node.measured.height || !s.edges.some((edge) => edge.target === id)) {
      return null;
    }
    return {
  ...
  const updateDimension = (attr: any) => (event: any) => {
    ...
    setNodes((nds) =>
      nds.map((n) => {
        if (n.id === '2-3') {
          const parentNode: any = nds.find((node) => node.id === '2-1');
          ...
          const currentNode: any = nds.find((node) => node.id === '2-3');
