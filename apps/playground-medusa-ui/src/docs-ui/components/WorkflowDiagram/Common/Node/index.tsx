  ...
  return (
    <Tooltip
      ...
      tooltipChildren={
        <>
          ...
          {step.when && (
            <span className="py-docs_0.25 block">
              Runs only if a <InlineCode>when</InlineCode> condition is satisfied.
            </span>
          )}
          {description && (
            <MarkdownContent allowedElements={['a', 'strong', 'code']} unwrapDisallowed={true}>
              {description}
            </MarkdownContent>
          )}
        </>
    ...
    >
      ...
      >
        <div
          className={clsx(
            'shadow-borders-base bg-medusa-bg-base flex w-fit',
            'rounded-docs_sm py-docs_0.125 px-docs_0.5 items-center',
            (step.type === 'hook' || step.when) && 'gap-x-docs_0.125',
          )}
          data-step-id={step.name}
        >
          {step.type === 'hook' && (
            <div className="text-medusa-tag-orange-icon flex size-[20px] items-center justify-center">
              <Bolt />
            </div>
          )}
          {step.when && (
            <div className="text-medusa-tag-green-icon flex size-[20px] items-center justify-center">
              <InformationCircle />
            </div>
          )}
