...
export const WorkflowDiagramCanvas = ({ workflow }: WorkflowDiagramCommonProps) => {
  ...
  return (
    <div className="rounded-docs_DEFAULT h-[200px] w-full">
      <div ref={dragConstraints} className="rounded-docs_DEFAULT relative size-full">
        <div className="rounded-docs_DEFAULT shadow-elevation-card-rest relative size-full overflow-hidden object-contain">
          <div>
            <motion.div
              ...
              <main className="size-full">
                <div className="absolute left-[1100px] top-[1100px] flex select-none items-start">
                  {Object.entries(clusters).map(([depth, cluster]) => {
                    ...
                    return <WorkflowDiagramCanvasDepth cluster={cluster} next={next} key={depth} />;
                  })}
                </div>
              </main>
            </motion.div>
        ...
        <div className="bg-medusa-bg-base shadow-borders-base text-medusa-fg-subtle bottom-docs_1 left-docs_1.5 rounded-docs_sm absolute flex h-[28px] items-center overflow-hidden">
          <div className="flex items-center">
            <button
              ...
              disabled={!canZoomIn}
              aria-label="Zoom in"
              className="disabled:text-medusa-fg-disabled transition-fg hover:bg-medusa-bg-base-hover active:bg-medusa-bg-base-pressed focus-visible:bg-medusa-bg-base-pressed p-docs_0.25 border-r outline-none"
            >
              <PlusMini />
            </button>
            <div>
              <DropdownMenu>
                <DropdownMenu.Trigger className="disabled:text-medusa-fg-disabled transition-fg hover:bg-medusa-bg-base-hover active:bg-medusa-bg-base-pressed focus-visible:bg-medusa-bg-base-pressed p-docs_0.25 flex w-[50px] items-center justify-center border-r outline-none">
                  <Text as="span" size="xsmall" leading="compact" className="select-none tabular-nums">
                    {Math.round(zoom * 100)}%
                  </Text>
