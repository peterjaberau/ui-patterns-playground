...
export function ComponentInfoMapProvider({
  ...
  const [componentInfoMap, setComponentInfoMap] = useState<Map<string, ComponentInfo>>(
  ...
  const addComponentInfo = useCallback((widgetId: string, componentInfo: ComponentInfo) => {
    setComponentInfoMap((prev) => {
      const newMap = new Map(prev);
  ...
  const updateComponentInfo = useCallback((widgetId: string, componentInfo: ComponentInfo) => {
    setComponentInfoMap((prev) => {
      const newMap = new Map(prev);
      newMap.set(widgetId, componentInfo);
      return newMap;
