...
const BaseNode: FC<any> = ({ id, data, children }) => {
  ...
  const LoopIndex = useMemo(() => {
    ...
    if (text) {
      return (
        <div
          className={cn(
            'system-xs-medium text-text-tertiary mr-2',
    ...
    >
      <div
        className={cn(
          ...
          data.type !== BlockEnum.Iteration && data.type !== BlockEnum.Loop && 'bg-workflow-block-bg w-[240px]',
          (data.type === BlockEnum.Iteration || data.type === BlockEnum.Loop) &&
            'border-workflow-block-border bg-workflow-block-bg-transparent flex h-full w-full flex-col',
      ...
      >
              ...
              <Tooltip
                popupContent={
                  <div className="w-[180px]">
                    <div className="font-extrabold">{t('workflow.nodes.iteration.parallelModeEnableTitle')}</div>
                    {t('workflow.nodes.iteration.parallelModeEnableDesc')}
                  </div>
                }
              >
                <div className="system-2xs-medium-uppercase border-text-warning text-text-warning ml-1 flex items-center justify-center rounded-[5px] border-[1px] px-[5px] py-[3px]">
        ...
        {data.type !== BlockEnum.Iteration && data.type !== BlockEnum.Loop && cloneElement(children, { id, data })}
        {(data.type === BlockEnum.Iteration || data.type === BlockEnum.Loop) && (
          <div className="grow pb-1 pl-1 pr-1">{cloneElement(children, { id, data })}</div>
        )}
