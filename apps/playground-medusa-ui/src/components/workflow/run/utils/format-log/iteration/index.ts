...
const format = (list: NodeTracing[], t: any): NodeTracing[] => {
  const iterationNodeIds = list.filter((item) => item.node_type === BlockEnum.Iteration).map((item) => item.node_id);
  const iterationChildrenNodeIds = list
    .filter(
      (item) =>
        item.execution_metadata?.iteration_id && iterationNodeIds.includes(item.execution_metadata.iteration_id),
    )
