...
const NodePanel: FC<Props> = ({
  ...
  const getTime = (time: number) => {
    ...
    if (time > 60) return `${Number.parseInt(Math.round(time / 60).toString())} m ${(time % 60).toFixed(3)} s`;
    return `${time.toFixed(3)} s`;
  };

  const getTokenCount = (tokens: number) => {
    if (tokens < 1000) return tokens;
    if (tokens >= 1000 && tokens < 1000000) return `${Number.parseFloat((tokens / 1000).toFixed(3))}K`;
  ...
  return (
    <div className={cn('px-2 py-1', className)}>
      <div className="border-components-panel-border bg-background-default shadow-xs group rounded-[10px] border transition-all hover:shadow-md">
        <div
          className={cn(
        ...
        >
          ...
          />
          <div
            className={cn('system-xs-semibold-uppercase text-text-secondary grow truncate', hideInfo && '!text-xs')}
          ...
          {nodeInfo.status === 'stopped' && (
            <RiAlertFill
              className={cn('text-text-warning-secondary ml-2 h-4 w-4 shrink-0', inMessage && 'h-3.5 w-3.5')}
            />
          )}
        ...
        {!collapseState && !hideProcessDetail && (
          <div className="px-1 pb-1">
            ...
            {isIterationNode && !notShowIterationNav && onShowIterationDetail && (
              <IterationLogTrigger nodeInfo={nodeInfo} onShowIterationResultList={onShowIterationDetail} />
            )}
            {/* The nav to the Loop detail */}
            {isLoopNode && !notShowLoopNav && onShowLoopDetail && (
              <LoopLogTrigger nodeInfo={nodeInfo} onShowLoopResultList={onShowLoopDetail} />
            )}
            ...
            <div className={cn('mb-1', hideInfo && '!px-2 !py-0.5')}>
              ...
              {nodeInfo.status === 'failed' && <StatusContainer status="failed">{nodeInfo.error}</StatusContainer>}
              {nodeInfo.status === 'retry' && <StatusContainer status="failed">{nodeInfo.error}</StatusContainer>}
            </div>
            {nodeInfo.inputs && (
