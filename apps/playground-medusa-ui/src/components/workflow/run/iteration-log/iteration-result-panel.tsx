...
import { Iteration } from '@/components/base/icons/src/vender/workflow';
...
const IterationResultPanel: FC<Props> = ({ list, onBack, iterDurationMap }) => {
  ...
  const iterationStatusShow = (index: number, iteration: NodeTracing[], iterDurationMap?: IterationDurationMap) => {
    ...
    return (
      <>
        {hasDurationMap && (
          <div className="system-xs-regular text-text-tertiary">{countIterDuration(iteration, iterDurationMap)}</div>
        )}
        <RiArrowRightSLine
          className={cn(
      ...
      <div className="bg-components-panel-bg p-2">
        {list.map((iteration, index) => (
          <div key={index} className={cn('bg-background-section-burn mb-1 overflow-hidden rounded-xl border-none')}>
            ...
            {expandedIterations[index] && <div className="bg-divider-subtle h-px grow"></div>}
            <div
              className={cn(
