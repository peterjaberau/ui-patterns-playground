    ...
    >
      ...
      <div className="flex grow flex-col gap-y-0.5 overflow-hidden">
        <div className="mr-6 flex h-5 items-center gap-x-1">
          ...
          >
            {isDraft
              ? t('workflow.versionHistory.currentDraft')
              : item.marked_name || t('workflow.versionHistory.defaultName')}
          </div>
          {isLatest && (
            <div className="system-2xs-medium-uppercase border-text-accent-secondary bg-components-badge-bg-dimm text-text-accent-secondary flex h-5 shrink-0 items-center rounded-md border px-[5px]">
              {t('workflow.versionHistory.latest')}
            </div>
          )}
        </div>
        {!isDraft && (
          <div className="system-xs-regular text-text-secondary break-words">{item.marked_comment || ''}</div>
        )}
        {!isDraft && (
          <div className="system-xs-regular text-text-tertiary truncate">
