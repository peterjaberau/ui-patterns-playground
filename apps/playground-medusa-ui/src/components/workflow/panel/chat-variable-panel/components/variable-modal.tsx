...
import ObjectValueList from '@workflow/panel/chat-variable-panel/components/object-value-list';
import { DEFAULT_OBJECT_VALUE } from '@workflow/panel/chat-variable-panel/components/object-value-item';
import ArrayValueList from '@workflow/panel/chat-variable-panel/components/array-value-list';
...
const ChatVariableModal = ({ chatVar, onClose, onSave }: ModalPropsType) => {
  ...
  const editorMinHeight = useMemo(() => {
    if (type === ChatVarType.ArrayObject) return '240px';
    return '120px';
  }, [type]);
  ...
  const getObjectValue = useCallback(() => {
    if (!chatVar || Object.keys(chatVar.value).length === 0) return [DEFAULT_OBJECT_VALUE];
  ...
  const formatValueFromObject = useCallback((list: ObjectValueItem[]) => {
    return list.reduce((acc: any, curr) => {
      if (curr.key) acc[curr.key] = curr.value || null;
      return acc;
    }, {});
  }, []);
  ...
  const handleTypeChange = (v: ChatVarType) => {
    ...
    if (v === ChatVarType.ArrayObject) setEditInJSON(true);
    if (v === ChatVarType.String || v === ChatVarType.Number || v === ChatVarType.Object) setEditInJSON(false);
    setType(v);
  };
  ...
  const handleEditorChange = (editInJSON: boolean) => {
    ...
    if (type === ChatVarType.ArrayString || type === ChatVarType.ArrayNumber) {
      if (editInJSON) {
        ...
        setValue(newValue);
        if (!editorContent) setEditorContent(JSON.stringify(newValue));
      } else {
  ...
  return (
    <div
      className={cn(
        'border-components-panel-border bg-components-panel-bg flex h-full w-[360px] flex-col rounded-2xl border-[0.5px] shadow-2xl',
        type === ChatVarType.Object && 'w-[480px]',
      )}
    >
      <div className="system-xl-semibold text-text-primary mb-3 flex shrink-0 items-center justify-between p-4 pb-0">
      ...
      <div className="max-h-[480px] overflow-y-auto px-4 py-2">
        {/* name */}
        <div className="mb-4">
          <div className="system-sm-semibold text-text-secondary mb-1 flex h-6 items-center">
        ...
        <div className="mb-4">
          ...
          <div className="flex">
            <VariableTypeSelector value={type} list={typeList} onSelect={handleTypeChange} popupClassName="w-[327px]" />
          </div>
        </div>
        {/* default value */}
        <div className="mb-4">
          <div className="system-sm-semibold text-text-secondary mb-1 flex h-6 items-center justify-between">
            ...
            {(type === ChatVarType.ArrayString || type === ChatVarType.ArrayNumber) && (
              ...
              >
                {editInJSON ? (
                  <RiInputField className="mr-1 h-3.5 w-3.5" />
                ) : (
                  <RiDraftLine className="mr-1 h-3.5 w-3.5" />
                )}
          ...
          <div className="flex">
            ...
            {type === ChatVarType.Object && !editInJSON && (
              <ObjectValueList list={objectValue} onChange={setObjectValue} />
            )}
            {type === ChatVarType.ArrayString && !editInJSON && (
              <ArrayValueList isString list={value || [undefined]} onChange={setValue} />
            )}
            {type === ChatVarType.ArrayNumber && !editInJSON && (
              <ArrayValueList isString={false} list={value || [undefined]} onChange={setValue} />
            )}
            {editInJSON && (
              <div
                className="bg-components-input-bg-normal w-full rounded-[10px] py-2 pl-3 pr-1"
        ...
        <div className="">
          ...
          <div className="flex">
            <textarea
              className="system-sm-regular placeholder:system-sm-regular bg-components-input-bg-normal caret-primary-600 placeholder:text-components-input-text-placeholder hover:border-components-input-border-hover hover:bg-components-input-bg-hover focus:border-components-input-border-active focus:bg-components-input-bg-active focus:shadow-xs block h-20 w-full resize-none appearance-none rounded-lg border border-transparent p-2 outline-none"
              value={des}
