    ...
    <div className="group/plugin hover:bg-state-base-hover flex rounded-lg py-1 pl-3 pr-1">
      ...
      <div className="ml-2 flex w-0 grow">
        <div className="w-0 grow">
          <div className="system-sm-medium text-text-primary h-4 truncate leading-4">
            {getLocalizedText(payload.label)}
          </div>
          <div className="system-xs-regular text-text-tertiary h-5 truncate leading-5">
            {getLocalizedText(payload.brief)}
          </div>
          <div className="system-xs-regular text-text-tertiary flex space-x-1">
            <div>{payload.org}</div>
            <div>·</div>
        ...
        <div
          className={cn(
            !open ? 'hidden' : 'flex',
            'system-xs-medium text-components-button-secondary-accent-text h-4 items-center space-x-1 group-hover/plugin:flex',
          )}
        >
          <div className="cursor-pointer px-1.5" onClick={showInstallModal}>
