import { Position, getConnectedEdges, getIncomers, getOutgoers } from 'reactflow';
import dagre from '@dagrejs/dagre';
import { v4 as uuid4 } from 'uuid';
import { cloneDeep, groupBy, isEqual, uniqBy } from 'lodash-es';
import type { Edge, InputVar, Node, ToolWithProvider, ValueSelector } from './types';
...
import { canFindTool, correctModelProvider } from '@utils/index';
import { CUSTOM_SIMPLE_NODE } from '@workflow/simple-node/constants';
...
export const preprocessNodesAndEdges = (nodes: Node[], edges: Edge[]) => {
  ...
  const newIterationStartNodes = [...iterationNodesWithStartNode, ...iterationNodesWithoutStartNode].map(
    (iterationNode, index) => {
...
export const initialNodes = (originNodes: Node[], originEdges: Edge[]) => {
  ...
  const iterationOrLoopNodeMap = nodes.reduce(
    (acc, node) => {
      if (node.parentId) {
        if (acc[node.parentId]) acc[node.parentId].push({ nodeId: node.id, nodeType: node.data.type });
...
export const getNodesConnectedSourceOrTargetHandleIdsMap = (
  ...
  changes.forEach((change) => {
    ...
    if (sourceNode) {
      nodesConnectedSourceOrTargetHandleIdsMap[sourceNode.id] = nodesConnectedSourceOrTargetHandleIdsMap[
        sourceNode.id
      ] || {
    ...
    if (targetNode) {
      nodesConnectedSourceOrTargetHandleIdsMap[targetNode.id] = nodesConnectedSourceOrTargetHandleIdsMap[
        targetNode.id
      ] || {
    ...
    if (sourceNode) {
      if (type === 'remove') {
        const index = nodesConnectedSourceOrTargetHandleIdsMap[sourceNode.id]._connectedSourceHandleIds.findIndex(
    ...
    if (targetNode) {
      if (type === 'remove') {
        const index = nodesConnectedSourceOrTargetHandleIdsMap[targetNode.id]._connectedTargetHandleIds.findIndex(
  ...
  const traverse = (root: Node, depth: number) => {
    ...
    if (outgoers.length) {
      outgoers.forEach((outgoer) => {
        ...
        if (outgoer.data.type === BlockEnum.Loop) list.push(...nodes.filter((node) => node.parentId === outgoer.id));
      ...
      if (root.data.type === BlockEnum.Iteration) list.push(...nodes.filter((node) => node.parentId === root.id));
  ...
  const traverse = (firstNodeHandle: NodeHandle) => {
    ...
    while (nextHandles.length) {
      ...
      const connectedEdges = edges.filter(
        (edge) => edge.source === currentNode.id && edge.sourceHandle === currentHandle,
      );
      ...
      if (nodeEdgesSet[currentNodeHandleKey]?.size > 0 && incomers.length > 1) {
        ...
        for (const item of totalEdgesSet) {
          if (!streamInfo[currentNodeHandleKey].downstreamEdges.has(item)) newSet.add(item);
        }
      ...
      outgoers.forEach((outgoer) => {
        ...
        if (!outgoerConnectedEdges.length) nextHandles.push({ node: outgoer, handle: 'source' });
        ...
        if (nodeEdgesSet[currentNodeHandleKey]) {
          for (const item of nodeEdgesSet[currentNodeHandleKey]) nodeEdgesSet[outgoerKey].add(item);
        }
        ...
        if (connectedEdgesLength > 1) {
          ...
          if (!parallelListItem.parallelNodeId) parallelListItem.parallelNodeId = currentNode.id;
...
export const getEdgeColor = (nodeRunningStatus?: NodeRunningStatus, isFailBranch?: boolean) => {
  if (nodeRunningStatus === NodeRunningStatus.Succeeded) return 'var(--color-workflow-link-line-success-handle)';

  if (nodeRunningStatus === NodeRunningStatus.Failed) return 'var(--color-workflow-link-line-error-handle)';

  if (nodeRunningStatus === NodeRunningStatus.Exception) return 'var(--color-workflow-link-line-failure-handle)';
...
export const isExceptionVariable = (variable: string, nodeType?: BlockEnum) => {
  if ((variable === 'error_message' || variable === 'error_type') && hasErrorHandleNode(nodeType)) return true;
