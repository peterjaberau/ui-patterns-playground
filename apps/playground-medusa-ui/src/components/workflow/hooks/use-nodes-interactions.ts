...
import { getConnectedEdges, getOutgoers, useReactFlow, useStoreApi } from 'reactflow';
...
export const useNodesInteractions = () => {
  ...
  const handleNodeDragStart = useCallback<NodeDragHandler>(
    (_, node) => {
      ...
      if (node.type === CUSTOM_ITERATION_START_NODE || node.type === CUSTOM_NOTE_NODE) return;

      if (node.type === CUSTOM_LOOP_START_NODE || node.type === CUSTOM_NOTE_NODE) return;
  ...
  const handleNodeDrag = useCallback<NodeDragHandler>(
    (e, node: Node) => {
      ...
      const newNodes = produce(nodes, (draft) => {
        ...
        if (showVerticalHelpLineNodesLength > 0) currentNode.position.x = showVerticalHelpLineNodes[0].position.x;
        else if (restrictPosition.x !== undefined) currentNode.position.x = restrictPosition.x;
        else if (restrictLoopPosition.x !== undefined) currentNode.position.x = restrictLoopPosition.x;
        else currentNode.position.x = node.position.x;

        if (showHorizontalHelpLineNodesLength > 0) currentNode.position.y = showHorizontalHelpLineNodes[0].position.y;
        else if (restrictPosition.y !== undefined) currentNode.position.y = restrictPosition.y;
        else if (restrictLoopPosition.y !== undefined) currentNode.position.y = restrictLoopPosition.y;
  ...
  const handleNodeEnter = useCallback<NodeMouseHandler>(
    (_, node) => {
      ...
      if (node.type === CUSTOM_NOTE_NODE || node.type === CUSTOM_ITERATION_START_NODE) return;
          ...
          const newNodes = produce(nodes, (draft) => {
            draft.forEach((n) => {
              if (
                n.id === node.id &&
                fromType === 'source' &&
                (node.data.type === BlockEnum.VariableAssigner || node.data.type === BlockEnum.VariableAggregator)
              ) {
      ...
      for (let i = 0; i < connectedEdges.length; i++) {
        const sourceConnectedEdges = getConnectedEdges([{ id: connectedEdges[i].source } as Node], edges).filter(
          (edge) => edge.source === connectedEdges[i].source && edge.sourceHandle === connectedEdges[i].sourceHandle,
        );
      ...
      if (uniqTargetNodes.length > 1) {
        const newNodes = produce(nodes, (draft) => {
          draft.forEach((n) => {
            if (uniqTargetNodes.some((targetNode) => n.id === targetNode.id)) n.data._inParallelHovering = true;
          });
        });
        setNodes(newNodes);
      }
  ...
  const handleNodeLeave = useCallback<NodeMouseHandler>(
    (_, node) => {
      ...
      if (node.type === CUSTOM_NOTE_NODE || node.type === CUSTOM_LOOP_START_NODE) return;
  ...
  const handleNodeConnect = useCallback<OnConnect>(
    ({ source, sourceHandle, target, targetHandle }) => {
      ...
      if (sourceNode?.type === CUSTOM_NOTE_NODE || targetNode?.type === CUSTOM_NOTE_NODE) return;
      ...
      if (checkNestedParallelLimit(newNodes, newEdges, targetNode?.parentId)) {
        ...
        const { setConnectingNodePayload, setEnteringNodePayload } = workflowStore.getState();
  ...
  const handleNodeConnectEnd = useCallback<OnConnectEnd>(
    (e: any) => {
      ...
      const { connectingNodePayload, setConnectingNodePayload, enteringNodePayload, setEnteringNodePayload } =
      ...
      if (connectingNodePayload && enteringNodePayload) {
        ...
        if (
          ...
          (toNode.data.type === BlockEnum.VariableAssigner || toNode.data.type === BlockEnum.VariableAggregator)
        ) {
  ...
  const handleNodeAdd = useCallback<OnNodeAdd>(
    (
      { nodeType, sourceHandle = 'source', targetHandle = 'target', toolDefaultValue },
      ...
      const { newNode, newIterationStartNode, newLoopStartNode } = generateNewNode({
        ...
        data: {
          ...
          _showAddVariablePopup:
            (nodeType === BlockEnum.VariableAssigner || nodeType === BlockEnum.VariableAggregator) && !!prevNodeId,
  ...
  const handleNodeChange = useCallback(
    (currentNodeId: string, nodeType: BlockEnum, sourceHandle: string, toolDefaultValue?: ToolDefaultValue) => {
      if (getNodesReadOnly()) return;
    ...
    (nodeId?: string) => {
      ...
      if (nodeId) {
        ...
        const selectedNode = nodes.find(
          (node) => node.data.selected && node.data.type !== BlockEnum.Start && node.data.type !== BlockEnum.LoopEnd,
        );
  ...
  const handleNodeResize = useCallback(
    (nodeId: string, params: ResizeParamsWithDirection) => {
      ...
      if (rightNode! && bottomNode!) {
        ...
        if (height < bottomNode.position.y + bottomNode.height! + paddingMap.bottom) return;
      }
      const newNodes = produce(nodes, (draft) => {
        draft.forEach((n) => {
          if (n.id === nodeId) {
