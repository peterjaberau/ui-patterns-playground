  ...
  return (
    ...
    >
      <PortalToFollowElemTrigger asChild>
        <div
          className={`border-components-actionbar-border bg-components-actionbar-bg hover:bg-state-base-hover h-9 cursor-pointer rounded-lg border-[0.5px] p-0.5 text-[13px] shadow-lg backdrop-blur-[5px] ${workflowReadOnly && '!cursor-not-allowed opacity-50'} `}
        >
          <div className={cn('flex h-8 w-[98px] items-center justify-between rounded-lg')}>
            <TipPopup title={t('workflow.operator.zoomOut')} shortcuts={['ctrl', '-']}>
              <div
            ...
            <div
              ...
              className={cn('system-sm-medium text-text-tertiary hover:text-text-secondary w-[34px]')}
            >
              {Number.parseFloat(`${zoom * 100}`).toFixed(0)}%
            </div>
            <TipPopup title={t('workflow.operator.zoomIn')} shortcuts={['ctrl', '+']}>
              <div
            ...
            <Fragment key={i}>
              ...
              <div className="p-1">
                {options.map((option) => (
                  <div
                    key={option.key}
                    className="system-md-regular text-text-secondary hover:bg-state-base-hover flex h-8 cursor-pointer items-center justify-between space-x-1 rounded-lg py-1.5 pl-3 pr-2"
                    onClick={() => handleZoom(option.key)}
                  >
                    <span>{option.text}</span>
                    <div className="flex items-center space-x-0.5">
                      {option.key === ZoomType.zoomToFit && <ShortcutsName keys={['ctrl', '1']} />}
