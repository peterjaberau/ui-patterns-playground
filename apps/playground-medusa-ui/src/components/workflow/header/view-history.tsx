...
import { ClockPlay, ClockPlaySlim } from '@/components/base/icons/src/vender/line/time';
...
const ViewHistory = ({ withText }: ViewHistoryProps) => {
  ...
  const { data: runList, isLoading: runListLoading } = useSWR(
    appDetail && !isChatMode && open ? `/apps/${appDetail.id}/workflow-runs` : null,
    fetchWorkflowRunHistory,
  );
  const { data: chatList, isLoading: chatListLoading } = useSWR(
    appDetail && isChatMode && open ? `/apps/${appDetail.id}/advanced-chat/workflow-runs` : null,
    fetchChatRunHistory,
  );
      ...
      <PortalToFollowElemTrigger onClick={() => setOpen((v) => !v)}>
        ...
        {!withText && (
          <Tooltip popupContent={t('workflow.common.viewRunHistory')}>
            <div
              className={cn(
                'hover:bg-state-accent-hover group flex h-7 w-7 cursor-pointer items-center justify-center rounded-md',
            ...
            >
              <ClockPlay
                className={cn(
                  'group-hover:text-components-button-secondary-accent-text h-4 w-4',
                  open ? 'text-components-button-secondary-accent-text' : 'text-components-button-ghost-text',
                )}
              />
            </div>
          </Tooltip>
            ...
            <div className="p-2">
              {!data?.data.length && (
                <div className="py-12">
                  ...
                  <div className="text-center text-[13px] text-gray-400">{t('workflow.common.notRunning')}</div>
                </div>
              )}
              {data?.data.map((item) => (
                <div
