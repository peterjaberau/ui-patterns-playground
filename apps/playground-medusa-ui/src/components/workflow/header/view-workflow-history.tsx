  ...
  const calculateChangeList: ChangeHistoryList = useMemo(() => {
    ...
    const historyData = {
      ...
      futureStates: filterList(
        [...futureStates, !pastStates.length && !futureStates.length ? undefined : store.getState()].filter(Boolean),
        0,
        true,
      ),
      statesCount: 0,
  ...
  return (
    ...
    >
        ...
        <TipPopup title={t('workflow.changeHistory.title')}>
          <div
            className={classNames(
              'text-text-tertiary hover:bg-state-base-hover hover:text-text-secondary flex h-8 w-8 cursor-pointer items-center justify-center rounded-md',
      ...
      <PortalToFollowElemContent className="z-[12]">
        <div className="border-components-panel-border bg-components-panel-bg-blur ml-2 flex min-w-[240px] max-w-[360px] flex-col overflow-y-auto rounded-xl border-[0.5px] shadow-xl backdrop-blur-[5px]">
          <div className="sticky top-0 flex items-center justify-between px-4 pt-3">
            <div className="system-mg-regular text-text-secondary grow">{t('workflow.changeHistory.title')}</div>
            <div
              ...
              <div className="flex flex-col">
                {calculateChangeList.futureStates.map((item: ChangeHistoryEntry) => (
                  <div
                    key={item?.index}
                    className={cn(
                      'text-text-secondary hover:bg-state-base-hover mb-0.5 flex cursor-pointer rounded-lg px-2 py-[7px]',
                  ...
                  >
                    <div>
                      <div
                        className={cn('text-text-secondary flex items-center text-[13px] font-medium leading-[18px]')}
                      >
