      ...
      <PortalToFollowElemTrigger onClick={() => !disabled && setOpen((v) => !v)}>
        <div
          className={classNames(
            'bg-components-input-bg-normal flex items-center gap-0.5 rounded-lg px-2 py-1',
            disabled
              ? '!bg-components-input-bg-disabled cursor-not-allowed'
              : 'hover:bg-state-base-hover-alt cursor-pointer',
        ...
        >
          <div className="flex items-center p-1">
            <span
              className={`system-sm-regular overflow-hidden truncate text-ellipsis ${selectedItem ? 'text-components-input-text-filled' : 'text-components-input-text-disabled'}`}
            >
              {selectedItem?.name
                ? t(`${i18nPrefix}.operations.${selectedItem?.name}`)
                : t(`${i18nPrefix}.operations.title`)}
            </span>
          </div>
          <RiArrowDownSLine
            className={`text-text-quaternary h-4 w-4 ${disabled && 'text-components-input-text-placeholder'} ${open && 'text-text-secondary'}`}
          />
        </div>
      </PortalToFollowElemTrigger>
        ...
        <div className="border-components-panel-border bg-components-panel-bg-blur flex w-[140px] flex-col items-start rounded-xl border-[0.5px] shadow-lg">
          <div className="flex flex-col items-start self-stretch p-1">
            <div className="flex items-start self-stretch px-3 pb-0.5 pt-1">
              <div className="system-xs-medium-uppercase text-text-tertiary flex grow">
                {t(`${i18nPrefix}.operations.title`)}
              </div>
            </div>
            {items.map((item) =>
              item.value === 'divider' ? (
                ...
                <div
                  ...
                  className={classNames(
                    ...
                    'hover:bg-state-base-hover cursor-pointer',
                  )}
                  onClick={() => {
                    onSelect(item);
                    setOpen(false);
                ...
                >
                  <div className="flex min-h-5 grow items-center gap-1 px-1">
                    <span className={'system-sm-medium text-text-secondary flex grow'}>
