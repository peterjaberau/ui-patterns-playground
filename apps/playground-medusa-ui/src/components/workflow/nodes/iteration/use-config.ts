...
const useConfig = (id: string, payload: IterationNodeType) => {
  ...
  const handleOutputVarChange = useCallback(
    (output: ValueSelector | string, _varKindType: VarKindType, varInfo?: Var) => {
      const newInputs = produce(inputs, (draft) => {
  ...
  const [isShowIterationDetail, { setTrue: doShowIterationDetail, setFalse: doHideIterationDetail }] =
    useBoolean(false);
  ...
  const changeParallel = useCallback(
    (value: boolean) => {
      const newInputs = produce(inputs, (draft) => {
  ...
  const changeParallelNums = useCallback(
    (num: number) => {
      const newInputs = produce(inputs, (draft) => {
