...
const VarGroupItem: FC<Props> = ({
  ...
  const handleAddVariable = useCallback(
    (value: ValueSelector | string, _varKindType: VarKindType, varInfo?: Var) => {
      ...
      if (chosenVariables.some((item) => item.join('.') === (value as ValueSelector).join('.'))) return;
  ...
  const handleListChange = useCallback(
    (newList: ValueSelector[], changedItem?: ValueSelector) => {
      if (changedItem) {
        const chosenVariables = payload.variables;
        if (chosenVariables.some((item) => item.join('.') === (changedItem as ValueSelector).join('.'))) return;
      }
  ...
  const [isEditGroupName, { setTrue: setEditGroupName, setFalse: setNotEditGroupName }] = useBoolean(false);
        ...
        groupEnabled ? (
          <div className="flex items-center">
            <div className="flex items-center !normal-case">
              ...
              {!isEditGroupName ? (
                <div
                  className="system-sm-semibold text-text-secondary flex h-6 cursor-text items-center rounded-lg px-1 hover:bg-gray-100"
      ...
      operations={
        <div className="flex h-6 items-center space-x-2">
          {payload.variables.length > 0 && (
            <div className="system-2xs-medium-uppercase border-divider-deep text-text-tertiary flex h-[18px] items-center rounded-[5px] border px-1">
              {payload.output_type}
            </div>
          )}
          {!readOnly ? (
