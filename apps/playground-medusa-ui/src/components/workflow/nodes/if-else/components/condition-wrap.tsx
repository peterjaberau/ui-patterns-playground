...
import { PortalSelect as Select } from '@/components/base/select';
      ...
      >
        {cases.map((item, index) => (
          <div key={item.case_id}>
            ...
            {!isSubVariable && <div className="bg-divider-subtle mx-3 my-2 h-[1px]"></div>}
          </div>
        ))}
      </ReactSortable>
      {cases.length === 0 && (
        <Button size="small" disabled={readOnly} onClick={() => handleAddSubVariableCondition?.(caseId!, conditionId!)}>
          <RiAddLine className="mr-1 h-3.5 w-3.5" />
          {t('workflow.nodes.ifElse.addSubVariable')}
        </Button>
      )}
