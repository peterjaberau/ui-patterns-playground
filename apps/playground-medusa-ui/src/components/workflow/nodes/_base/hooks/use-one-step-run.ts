...
import LLMDefault from '@workflow/nodes/llm/default';
import KnowledgeRetrievalDefault from '@workflow/nodes/knowledge-retrieval/default';
import IfElseDefault from '@workflow/nodes/if-else/default';
import CodeDefault from '@workflow/nodes/code/default';
import TemplateTransformDefault from '@workflow/nodes/template-transform/default';
import QuestionClassifyDefault from '@workflow/nodes/question-classifier/default';
import HTTPDefault from '@workflow/nodes/http/default';
import ToolDefault from '@workflow/nodes/tool/default';
import VariableAssigner from '@workflow/nodes/variable-assigner/default';
import Assigner from '@workflow/nodes/assigner/default';
import ParameterExtractorDefault from '@workflow/nodes/parameter-extractor/default';
import IterationDefault from '@workflow/nodes/iteration/default';
import DocumentExtractorDefault from '@workflow/nodes/document-extractor/default';
import LoopDefault from '@workflow/nodes/loop/default';
...
const useOneStepRun = <T>({
  ...
  const getVar = (valueSelector: ValueSelector): Var | undefined => {
    ...
    if (isSystem) return targetVar.vars.find((item) => item.variable.split('.')[1] === valueSelector[1]);
    ...
    for (let i = 1; i < valueSelector.length; i++) {
      ...
      if (Array.isArray(curr)) curr = curr.find((v: any) => v.variable.replace('conversation.', '') === key);
        ...
        ssePost(
          ...
          { body: { inputs: submitData } },
          {
            ...
            onIterationNext: () => {
              ...
              if (_iterationResult.length >= iterationTimes!) return _iterationResult.length >= iterationTimes!;
            },
