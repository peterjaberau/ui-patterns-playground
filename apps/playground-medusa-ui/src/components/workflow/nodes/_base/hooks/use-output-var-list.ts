...
import { BlockEnum, VarType } from '@workflow/types';
import { useWorkflow } from '@workflow/hooks';
import { ErrorHandleTypeEnum } from '@workflow/nodes/_base/components/error-handle/types';
import { getDefaultValue } from '@workflow/nodes/_base/components/error-handle/utils';
...
function useOutputVarList<T>({
  ...
  const handleVarsChange = useCallback(
    (newVars: OutputVar, changedIndex?: number, newKey?: string) => {
      ...
      if (newKey) handleOutVarRenameChange(id, [id, outputKeyOrders[changedIndex!]], [id, newKey]);
    },
