...
const VariableTag = ({ valueSelector, varType, isShort, availableNodes }: VariableTagProps) => {
  ...
  return (
    <Tooltip popupContent={!isValid && t('workflow.errorMsg.invalidVariable')}>
      <div
        className={cn(
          'border-[rgba(16, 2440,0.08)] shadow-xs inline-flex h-6 max-w-full items-center rounded-md border-[0.5px] bg-white px-1.5 text-xs',
      ...
      >
        {!isEnv && !isChatVar && (
          <>
            {node && (
              <>
                <VarBlockIcon type={node.data.type || BlockEnum.Start} className="mr-0.5" />
                <div className="text-text-secondary max-w-[60px] truncate font-medium" title={node?.data.title}>
                  {node?.data.title}
                </div>
              </>
            )}
            ...
            <Variable02
              className={cn('text-text-accent mr-0.5 h-3.5 w-3.5 shrink-0', isException && 'text-text-warning')}
            />
          </>
        )}
        ...
        <div
          className={cn(
            'text-text-accent ml-0.5 truncate font-medium',
            (isEnv || isChatVar) && 'text-text-secondary',
        ...
        {!isShort && varType && <div className="text-text-tertiary ml-0.5 shrink-0">{capitalize(varType)}</div>}
