...
const ErrorHandle = ({ id, data }: ErrorHandleProps) => {
  ...
  const getHandleErrorHandleTypeChange = useCallback(
    (data: CommonNodeType) => {
  ...
  const getHandleFormChange = useCallback(
    (data: CommonNodeType) => {
        ...
        >
          <>
            ...
            {error_strategy === ErrorHandleTypeEnum.defaultValue && !collapsed && !!default_value?.length && (
              <DefaultValue forms={default_value} onFormChange={getHandleFormChange(data)} />
            )}
          </>
        </Collapse>
      </div>
