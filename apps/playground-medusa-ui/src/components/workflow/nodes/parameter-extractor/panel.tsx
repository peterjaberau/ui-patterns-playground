...
import { InputVarType, type NodePanelProps } from '@workflow/types';
...
import type { Props as FormProps } from '@workflow/nodes/_base/components/before-run-form/form';
  ...
  return (
    <div className="pt-2">
      <div className="space-y-4 px-4">
        ...
        >
          <ExtractParameter readonly={readOnly} list={inputs.parameters || []} onChange={handleExactParamsChange} />
        </Field>
        <Editor
          title={
            <div className="flex items-center space-x-1">
              ...
              <Tooltip
                popupContent={<div className="w-[120px]">{t(`${i18nPrefix}.instructionTip`)}</div>}
      ...
      <FieldCollapse title={t(`${i18nPrefix}.advancedSetting`)}>
        <>
          ...
          {isSupportFunctionCall && (
            <div className="mt-2">
              <ReasoningModePicker type={inputs.reasoning_mode} onChange={handleReasoningModeChange} />
            </div>
          )}
        </>
      </FieldCollapse>
            ...
            <OutputVars>
              <>
                {inputs.parameters.map((param, index) => (
                  <VarItem key={index} name={param.name} type={param.type} description={param.description} />
                ))}
                <VarItem name="__is_success" type={VarType.number} description={t(`${i18nPrefix}.isSuccess`)} />
                <VarItem name="__reason" type={VarType.string} description={t(`${i18nPrefix}.errorReason`)} />
              </>
            </OutputVars>
          </div>
        </>
