...
import { FILE_TYPE_OPTIONS, TRANSFER_METHOD } from '@workflow/nodes/constants';
...
const FilterCondition: FC<Props> = ({
  ...
  return (
    <div>
      {hasSubVariable && (
        <SubVariablePicker className="mb-2" value={condition.key} onChange={handleSubVariableChange} />
      )}
        ...
        {!comparisonOperatorNotRequireValue(condition.comparison_operator) && (
          <>
            ...
            {!isSelect && (
              <Input
                type={
                  (hasSubVariable && condition.key === 'size') || (!hasSubVariable && varType === VarType.number)
                    ? 'number'
                    : 'text'
                }
                className="grow"
