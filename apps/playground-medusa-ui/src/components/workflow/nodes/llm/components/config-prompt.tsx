...
const ConfigPrompt: FC<Props> = ({
  ...
  const canChooseSystemRole = (() => {
    if (isChatModel && Array.isArray(payload)) return !payload.find((item) => item.role === PromptRole.system);
    ...
    <div>
      {isChatModel && Array.isArray(payload) ? (
        <div>
          ...
          <AddButton className="mt-2" text={t(`${i18nPrefix}.addMessage`)} onClick={handleAddPrompt} />
        </div>
      ) : (
        <div>
          <Editor
            ...
            value={
              (payload as PromptItem).edition_type === EditionType.basic || !(payload as PromptItem).edition_type
