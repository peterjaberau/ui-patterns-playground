...
const MetadataIcon = ({ type, className }: MetadataIconProps) => {
  return (
    <>
      {type === MetadataFilteringVariableType.string && <RiTextSnippet className={cn('h-3.5 w-3.5', className)} />}
      {type === MetadataFilteringVariableType.number && <RiHashtag className={cn('h-3.5 w-3.5', className)} />}
      {type === MetadataFilteringVariableType.time && <RiTimeLine className={cn('h-3.5 w-3.5', className)} />}
    </>
  );
};
