    ...
    <div>
      <DropdownMenu open={openMenu} onOpenChange={setOpenMenu}>
        ...
        <DropdownMenu.Content className="min-w-[var(--radix-dropdown-menu-trigger-width)] max-w-[var(--radix-dropdown-menu-trigger-width)]">
          ...
          <DropdownMenu.Separator />
          <DropdownMenu.Item asChild>
            <Link href="https://docs.medusajs.com" target="_blank">
              <BookOpen className="text-ui-fg-subtle mr-2" />
              'Documentation'
            </Link>
          </DropdownMenu.Item>
          <DropdownMenu.Item asChild>
            <Link href="https://medusajs.com/changelog/" target="_blank">
...
const UserBadge = () => {
  const [user, setUser]: any = useState(useMe.user);
  const [isPending, setIsPending]: any = useState(useMe.isPending);
  const [isError, setIsError]: any = useState(useMe.isError);
  const [error, setError]: any = useState(useMe.error);
      ...
      <RadixDialog.Portal>
        ...
        <RadixDialog.Content className="bg-ui-bg-subtle shadow-elevation-modal fixed left-[50%] top-[50%] flex h-full max-h-[612px] w-full max-w-[560px] translate-x-[-50%] translate-y-[-50%] flex-col divide-y overflow-hidden rounded-lg">
          <div className="flex flex-col gap-y-3 px-6 py-4">
            ...
            <div>
              <Input type="search" value={searchValue} onChange={(e) => onSearchValueChange(e.target.value)} />
            </div>
          </div>
          <div className="flex flex-col divide-y overflow-y-auto">
            {searchResults.map((shortcut, index) => {
              return (
                <div key={index} className="text-ui-fg-subtle flex items-center justify-between px-6 py-3">
                      ...
                      return (
                        <div className="flex items-center gap-x-1" key={index}>
                          ...
                          {index < (shortcut.keys.Mac?.length || 0) - 1 && (
                            <span className="txt-compact-xsmall text-ui-fg-subtle">{'then'}</span>
                          )}
                        </div>
                      );
                    })}
