...
const FilterChip = ({ hadPreviousValue, label, value, readonly, hasOperator, onRemove }: FilterChipProps) => {
      ...
      <div className="flex w-full items-center overflow-hidden">
        {hasOperator && !!(value || hadPreviousValue) && (
          <div className="border-r p-1 px-2">
            <Text size="small" weight="plus" leading="compact" className="text-ui-fg-muted">
              {t('general.is')}
            </Text>
          </div>
        )}
        {!!(value || hadPreviousValue) && (
          ...
          >
            <Text size="small" leading="compact" weight="plus" className="truncate text-nowrap">
              {value || '\u00A0'}
            </Text>
          </RadixPopover.Trigger>
        )}
