...
export const DataTableOrderBy = <TData,>({ keys, prefix }: DataTableOrderByProps<TData>) => {
  ...
  const getInitialState = (): { key?: string; dir: SortDirection } => {
    const orderValue = searchParams.get(param);
    if (!orderValue) return { key: undefined, dir: SortDirection.ASC };

    return {
      key: orderValue.startsWith('-') ? orderValue.substring(1) : orderValue,
      dir: orderValue.startsWith('-') ? SortDirection.DESC : SortDirection.ASC,
    };
  };
  ...
  const updateOrderParam = (newState: SortState) => {
    const newSearchParams = new URLSearchParams(searchParams.toString());

    if (!newState.key) {
      newSearchParams.delete(param);
    } else {
      const orderParam = newState.dir === SortDirection.ASC ? newState.key : `-${newState.key}`;
      newSearchParams.set(param, orderParam);
    }
      ...
      <DropdownMenu.Content className="z-[1]" align="end">
        <DropdownMenu.RadioGroup value={state.key} onValueChange={handleKeyChange}>
          {keys.map((key) => {
            ...
            return (
              <DropdownMenu.RadioItem key={stringKey} value={stringKey} onSelect={(event) => event.preventDefault()}>
                {key.label}
