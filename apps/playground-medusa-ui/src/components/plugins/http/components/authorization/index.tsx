...
import BaseInput from '@plugins/_ui/input';
...
const Authorization: FC<Props> = ({ nodeId, payload, onChange, isShow, onHide }) => {
  ...
  return (
    <Modal title={t(`${i18nPrefix}.authorization`)} isShow={isShow} onClose={onHide}>
      <div>
            ...
            <>
              ...
              {tempPayload.config?.type === APIType.custom && (
                <Field title={t(`${i18nPrefix}.header`)} isRequired>
                  <BaseInput value={tempPayload.config?.header || ''} onChange={handleAPIKeyOrHeaderChange('header')} />
                </Field>
              )}
              ...
              <Field title={t(`${i18nPrefix}.api-key-title`)} isRequired>
                <div className="flex">
                  <Input
                    ...
                    className={cn(
                      isFocus ? 'shadow-xs border-gray-300 bg-gray-50' : 'border-gray-100 bg-gray-100',
