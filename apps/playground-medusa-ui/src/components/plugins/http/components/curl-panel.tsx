...
import Modal from '@plugins/_ui/modal';
...
const parseCurl = (curlCommand: string): { node: HttpNodeType | null; error: string | null } => {
  ...
  for (let i = 1; i < args.length; i++) {
    ...
    switch (arg) {
      case '-X':
      case '--request':
        if (i + 1 >= args.length) return { node: null, error: 'Missing HTTP method after -X or --request.' };
        node.method = (args[++i].replace(/^['"]|['"]$/g, '') as Method) || Method.get;
        hasData = true;
        break;
      case '-H':
      case '--header':
        if (i + 1 >= args.length) return { node: null, error: 'Missing header value after -H or --header.' };
      ...
      case '-F':
      case '--form': {
        if (i + 1 >= args.length) return { node: null, error: 'Missing form data after -F or --form.' };
        ...
        if (!key) return { node: null, error: 'Invalid form data format.' };
      ...
      case '--json':
        if (i + 1 >= args.length) return { node: null, error: 'Missing JSON data after --json.' };
