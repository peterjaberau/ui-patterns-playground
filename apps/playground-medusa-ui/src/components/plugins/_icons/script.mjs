...
import IconBase from '@plugins/_icons/IconBase'
import type { IconData } from '@plugins/_icons/IconBase'
...
`.trim(),
  ...
  await writeFile(
    ...
    `${componentCSSRender({
      assetPath: path.posix.join('@plugins/_icons/assets', ...pathList.slice(2), entry),
    })}\n`,
  );
