...
const Toast = ({ type = 'info', size = 'md', message, children, className, customComponent }: IToastProps) => {
  ...
  return (
    <div
      className={classNames(
        className,
        'fixed z-[9999] mx-8 my-4 w-[360px] flex-grow overflow-hidden rounded-xl',
        size === 'md' ? 'p-3' : 'p-2',
        'border-components-panel-border-subtle bg-components-panel-bg-blur border shadow-sm',
        'top-0',
        'right-0',
      )}
    >
      ...
      <div className={`flex ${size === 'md' ? 'gap-1' : 'gap-0.5'}`}>
        <div className={`flex items-center justify-center ${size === 'md' ? 'p-0.5' : 'p-1'}`}>
          {type === 'success' && (
            <RiCheckboxCircleFill
              className={`${size === 'md' ? 'h-5 w-5' : 'h-4 w-4'} text-text-success`}
              aria-hidden="true"
            />
          )}
          {type === 'error' && (
            <RiErrorWarningFill
              className={`${size === 'md' ? 'h-5 w-5' : 'h-4 w-4'} text-text-destructive`}
              aria-hidden="true"
            />
          )}
          {type === 'warning' && (
            <RiAlertFill
              className={`${size === 'md' ? 'h-5 w-5' : 'h-4 w-4'} text-text-warning-secondary`}
              aria-hidden="true"
            />
          )}
          {type === 'info' && (
            <RiInformation2Fill
              className={`${size === 'md' ? 'h-5 w-5' : 'h-4 w-4'} text-text-accent`}
              aria-hidden="true"
            />
          )}
        </div>
