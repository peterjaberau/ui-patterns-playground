...
import { ImagePlus } from '@plugins/_icons/src/vender/line/images';
import { Link03 } from '@plugins/_icons/src/vender/line/general';
import {
  PortalToFollowElem,
  PortalToFollowElemContent,
  PortalToFollowElemTrigger,
} from '@plugins/_ui/portal-to-follow-elem';
import type { ImageFile, VisionSettings } from './types';
  ...
  return (
    <PortalToFollowElem open={open} onOpenChange={setOpen} placement="top-start">
      <PortalToFollowElemTrigger onClick={handleToggle}>
        <div
          className={`bg-components-button-tertiary-bg text-text-tertiary hover:bg-components-button-tertiary-bg-hover relative flex h-8 items-center justify-center rounded-lg px-3 text-xs ${disabled ? 'cursor-not-allowed' : 'cursor-pointer'} `}
        >
      ...
      <PortalToFollowElemContent className="z-10">
        <div className="border-components-panel-border bg-components-panel-bg w-[320px] rounded-lg border-[0.5px] p-2 shadow-lg">
...
const TextGenerationImageUploader: FC<TextGenerationImageUploaderProps> = ({ settings, onFilesChange }) => {
  ...
  const localUpload = (
    ...
    >
      {(hovering) => (
        <div
          className={`bg-components-button-tertiary-bg text-text-tertiary flex h-8 cursor-pointer items-center justify-center rounded-lg px-3 text-xs ${hovering && 'hover:bg-components-button-tertiary-bg-hover'} `}
        >
