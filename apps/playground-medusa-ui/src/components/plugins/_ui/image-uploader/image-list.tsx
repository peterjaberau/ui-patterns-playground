...
import { RefreshCcw01 } from '@plugins/_icons/src/vender/line/arrows';
import { AlertTriangle } from '@plugins/_icons/src/vender/solid/alertsAndFeedback';
import Tooltip from '@plugins/_ui/tooltip';
import type { ImageFile } from './types';
import { TransferMethod } from './types';
import ImagePreview from '@plugins/_ui/image-uploader/image-preview';
...
const ImageList: FC<ImageListProps> = ({
  ...
  return (
    <div className="flex flex-wrap">
      {list.map((item) => (
        <div key={item._id} className="group relative mr-1 rounded-lg border-[0.5px] border-black/5">
          {item.type === TransferMethod.local_file && item.progress !== 100 && (
            <>
              ...
              >
                {item.progress === -1 && (
                  <RefreshCcw01 className="h-5 w-5 text-white" onClick={() => onReUpload && onReUpload(item._id)} />
                )}
              </div>
              {item.progress > -1 && (
          ...
          {item.type === TransferMethod.remote_url && item.progress !== 100 && (
            ...
            >
              {item.progress > -1 && <RiLoader2Line className="h-5 w-5 animate-spin text-white" />}
              {item.progress === -1 && (
                <Tooltip popupContent={t('common.imageUploader.pasteImageLinkInvalid')}>
      ...
      {imagePreviewUrl && <ImagePreview url={imagePreviewUrl} onCancel={() => setImagePreviewUrl('')} title="" />}
    </div>
  );
};
