...
import ProgressCircle from '@plugins/_ui/progress-bar/progress-circle';
import { ReplayLine } from '@plugins/_icons/src/vender/other';
import ActionButton from '@plugins/_ui/action-button';
import Button from '@plugins/_ui/button';
import PdfPreview from '@plugins/_ui/file-uploader/dynamic-pdf-preview';
import AudioPreview from '@plugins/_ui/file-uploader/audio-preview';
import VideoPreview from '@plugins/_ui/file-uploader/video-preview';
    ...
    <>
      <div
        className={cn(
          ...
          uploadError && 'bg-state-destructive-hover-alt hover:border-state-destructive-border hover:border-[0.5px]',
        )}
      >
        {showDeleteAction && (
          <Button
        ...
        <div className="relative flex items-center justify-between">
          ...
          {progress >= 0 && !fileIsUploaded(file) && (
            <ProgressCircle percentage={progress} size={12} className="shrink-0" />
          )}
          {uploadError && <ReplayLine className="text-text-tertiary h-4 w-4" onClick={() => onReUpload?.(id)} />}
        </div>
      </div>
