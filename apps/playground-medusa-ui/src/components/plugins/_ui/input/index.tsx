...
const Input = ({
  ...
  return (
    <div className={cn('relative w-full', wrapperClassName)}>
      {showLeftIcon && (
        <RiSearchLine
          className={cn('text-components-input-text-placeholder absolute left-2 top-1/2 h-4 w-4 -translate-y-1/2')}
        />
      )}
      <input
        style={styleCss}
        className={cn(
          'bg-components-input-bg-normal text-components-input-text-filled caret-primary-600 placeholder:text-components-input-text-placeholder hover:border-components-input-border-hover hover:bg-components-input-bg-hover focus:border-components-input-border-active focus:bg-components-input-bg-active focus:shadow-xs w-full appearance-none border border-transparent py-[7px] outline-none',
          inputVariants({ size }),
          ...
          disabled &&
            'bg-components-input-bg-disabled text-components-input-text-filled-disabled hover:bg-components-input-bg-disabled cursor-not-allowed border-transparent hover:border-transparent',
          destructive &&
        ...
        placeholder={
          placeholder ?? (showLeftIcon ? t('common.operation.search') || '' : t('common.placeholder.input') || '')
        }
        value={value}
        onChange={onChange}
        disabled={disabled}
      ...
      {unit && (
        <div className="system-sm-regular text-text-tertiary absolute right-2 top-1/2 -translate-y-1/2">{unit}</div>
      )}
    </div>
  );
};
