...
const Tooltip: FC<TooltipProps> = ({
  ...
  return (
    <PortalToFollowElem open={disabled ? false : open} onOpenChange={setOpen} placement={position} offset={offset ?? 8}>
      <PortalToFollowElemTrigger
      ...
      >
        {children || (
          <div className={triggerClassName || 'h-3.5 w-3.5 shrink-0 p-[1px]'}>
            <RiQuestionLine className="text-text-quaternary hover:text-text-tertiary h-full w-full" />
          </div>
        )}
      </PortalToFollowElemTrigger>
      <PortalToFollowElemContent className="z-[9999]">
        {popupContent && (
          <div
            className={cn(
              !noDecoration &&
                'system-xs-regular bg-components-panel-bg text-text-tertiary relative break-words rounded-md px-3 py-2 shadow-lg',
              popupClassName,
            )}
