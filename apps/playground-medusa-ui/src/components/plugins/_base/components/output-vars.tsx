...
import { FieldCollapse } from '@plugins/_base/components/collapse';
...
export const VarItem: FC<VarItemProps> = ({ name, type, description, subItems }) => {
  return (
    <div className="py-1">
      <div className="flex items-center leading-[18px]">
        <div className="code-sm-semibold text-text-secondary">{name}</div>
        <div className="system-xs-regular text-text-tertiary ml-2 capitalize">{type}</div>
      </div>
        ...
        {subItems && (
          <div className="ml-2 border-l border-gray-200 pl-2">
            {subItems.map((item, index) => (
              <VarItem key={index} name={item.name} type={item.type} description={item.description} />
            ))}
          </div>
        )}
      </div>
