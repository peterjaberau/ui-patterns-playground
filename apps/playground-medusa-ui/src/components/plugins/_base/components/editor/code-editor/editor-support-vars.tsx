...
import VarReferenceVars from '@plugins/_base/components/variable/var-reference-vars';
...
const CodeEditor: FC<Props> = ({ availableVars, varList, onAddVar, ...editorProps }) => {
  ...
  const getVarName = (varValue: string[]) => {
    const existVar = varList.find(
      (v) => Array.isArray(v.value_selector) && v.value_selector.join('@@@') === varValue.join('@@@'),
    );
    if (existVar) {
      return {
  ...
  return (
    <div className={cn(editorProps.isExpand && 'h-full')}>
      <Editor {...editorProps} onMount={onEditorMounted} placeholder={t('workflow.common.jinjaEditorPlaceholder')!} />
      {isShowVarPicker && (
        <div
          ref={popupRef}
        ...
        >
          <VarReferenceVars hideSearch vars={availableVars} onChange={handleSelectVar} isSupportFileVar={false} />
        </div>
      )}
    </div>
  );
