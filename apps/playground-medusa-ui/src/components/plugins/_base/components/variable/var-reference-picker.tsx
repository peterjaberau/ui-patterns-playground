...
import cn from '@plugins/_utils/classnames';
import type { Node, NodeOutPutVar, ValueSelector, Var } from '@/app/components/workflow/types';
import type { CredentialFormSchema } from '@/app/components/header/account-setting/model-provider-page/declarations';
import { BlockEnum } from '@/app/components/workflow/types';
import { VarBlockIcon } from '@/app/components/workflow/block-icon';
import { Line3 } from '@plugins/_icons/src/public/common';
import { BubbleX, Env } from '@plugins/_icons/src/vender/line/others';
import { Variable02 } from '@plugins/_icons/src/vender/solid/development';
import {
  PortalToFollowElem,
...
import AddButton from '@plugins/_ui/button/add-button';
import Badge from '@plugins/_ui/badge';
import Tooltip from '@plugins/_ui/tooltip';
...
const VarReferencePicker: FC<Props> = ({
  ...
  const isIterationVar = useMemo(() => {
    ...
    if (value[0] === node?.parentId && ['item', 'index'].includes(value[1])) return true;
    return false;
  ...
  const [maxNodeNameWidth, maxVarNameWidth, maxTypeWidth] = (() => {
    ...
    const maxNodeNameWidth =
      PRIORITY_WIDTH + Math.floor(((outputVarNode?.title?.length || 0) / totalTextLength) * availableWidth);
        ...
        >
          <>
            {isAddBtnTrigger ? (
              ...
              <div
                ...
                className={cn(
                  ...
                  !isSupportConstantValue && 'bg-components-input-bg-normal rounded-lg p-1',
                ...
                {isSupportConstantValue ? (
                    ...
                    <TypeSelector
                      ...
                      trigger={
                        <div className="radius-md bg-components-input-bg-normal flex h-8 items-center px-2">
                          <div className="system-sm-regular text-components-input-text-filled mr-1">
                  ...
                  !hasValue && (
                    <div className="ml-1.5 mr-1">
                      <Variable02
                        className={`h-4 w-4 ${readonly ? 'text-components-input-text-disabled' : 'text-components-input-text-placeholder'}`}
                      />
                    </div>
                  )
                )}
                {isConstant ? (
                  ...
                  >
                    <div
                      ...
                      className={cn(
                        'h-full',
                        isSupportConstantValue && 'flex items-center rounded-lg bg-gray-100 py-1 pl-1',
                      )}
                    >
                        ...
                        >
                          {hasValue ? (
                            <>
                              {isShowNodeName && !isEnv && !isChatVar && (
                                <div className="flex items-center">
                                  <div className="h-3 px-[1px]">
                                    {outputVarNode?.type && (
                                      <VarBlockIcon className="!text-gray-900" type={outputVarNode.type} />
                                    )}
                                  </div>
                                  <div
                              ...
                              <div
                                className="system-xs-regular text-text-tertiary ml-0.5 truncate text-center capitalize"
                                title={type}
                                style={{
            ...
            {!hasValue && typePlaceHolder && (
              <Badge className="absolute right-2 top-1.5" text={typePlaceHolder} uppercase={false} />
            )}
          </>
        </WrapElem>
        <PortalToFollowElemContent
