    ...
    <div className="space-y-2">
      {list.map((item, index) => (
        <div className="flex items-center space-x-1" key={index}>
          ...
          />
          <VarTypePicker readonly={readonly} value={item.variable_type} onChange={handleVarTypeChange(index)} />
          <RemoveButton className="!bg-gray-100 !p-2 hover:!bg-gray-200" onClick={handleVarRemove(index)} />
        </div>
      ))}
    </div>
  );
