...
import { ChevronRight } from '@plugins/_icons/src/vender/line/arrows';
import {
  PortalToFollowElem,
  PortalToFollowElemContent,
  PortalToFollowElemTrigger,
} from '@plugins/_ui/portal-to-follow-elem';
import Input from '@plugins/_ui/input';
import { BubbleX, Env } from '@plugins/_icons/src/vender/line/others';
import { checkKeys } from '@/utils/var';
import { FILE_STRUCT } from '@/app/components/workflow/constants';
import { Loop } from '@plugins/_icons/src/vender/workflow';
...
const Item: FC<ItemProps> = ({
  ...
  const isObj =
    [VarType.object, VarType.file].includes(itemData.type) && itemData.children && itemData.children.length > 0;
  ...
  return (
    <PortalToFollowElem open={open} onOpenChange={() => {}} placement="left-start">
        ...
        >
          <div className="flex w-0 grow items-center">
            {!isEnv && !isChatVar && !isLoopVar && (
              <Variable02 className={cn('text-text-accent h-3.5 w-3.5 shrink-0', isException && 'text-text-warning')} />
            )}
            {isEnv && <Env className="text-util-colors-violet-violet-600 h-3.5 w-3.5 shrink-0" />}
            {isChatVar && <BubbleX className="text-util-colors-teal-teal-700 h-3.5 w-3.5 shrink-0" />}
            {isLoopVar && <Loop className="text-util-colors-cyan-cyan-500 h-3.5 w-3.5 shrink-0" />}
            {!isEnv && !isChatVar && (
              <div title={itemData.variable} className="system-sm-medium text-text-secondary ml-1 w-0 grow truncate">
                {itemData.variable}
              </div>
            )}
            {isEnv && (
            ...
            {isChatVar && (
              <div title={itemData.des} className="system-sm-medium text-text-secondary ml-1 w-0 grow truncate">
      ...
      {filteredVars.length > 0 ? (
        <div className={cn('max-h-[85vh] overflow-y-auto', maxHeightClass)}>
          {filteredVars.map((item, i) => (
            <div key={i}>
              <div
                className="system-xs-medium-uppercase text-text-tertiary truncate px-3 leading-[22px]"
                title={item.title}
              >
                {item.title}
              </div>
