...
const Field: FC<Props> = ({ className, title, isSubTitle, tooltip, children, operations, inline, supportFold }) => {
    ...
    <div className={cn(className, inline && 'flex w-full items-center justify-between')}>
      ...
      >
        <div className="flex h-6 items-center">
          ...
          {tooltip && <Tooltip popupContent={tooltip} popupClassName="ml-1" triggerClassName="w-4 h-4 ml-1" />}
        </div>
        <div className="flex">
          {operations && <div>{operations}</div>}
          {supportFold && (
            <RiArrowDownSLine
              className="text-text-tertiary h-4 w-4 cursor-pointer transition-transform"
