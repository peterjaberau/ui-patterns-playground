...
import { StopCircle } from '@plugins/_icons/src/vender/solid/mediaAndDevices';
import Split from '@/app/components/workflow/nodes/_base/components/split';
import { InputVarType, NodeRunningStatus } from '@/app/components/workflow/types';
import ResultPanel from '@/app/components/workflow/run/result-panel';
import Toast from '@plugins/_ui/toast';
import { TransferMethod } from '@/types/app';
import { getProcessedFiles } from '@plugins/_ui/file-uploader/utils';
...
function formatValue(value: string | any, type: InputVarType) {
  if (type === InputVarType.number) return Number.parseFloat(value);
  ...
  if (type === InputVarType.multiFiles) return getProcessedFiles(value);

  if (type === InputVarType.singleFile) {
    if (Array.isArray(value)) return getProcessedFiles(value);
      ...
      form.inputs.forEach((input) => {
        ...
        if (
          ...
          errMsg = t('workflow.errorMsg.fieldRequired', {
            field: typeof input.label === 'object' ? input.label.variable : input.label,
          });

        if (!errMsg && (input.type === InputVarType.singleFile || input.type === InputVarType.multiFiles) && value) {
          let fileIsUploading = false;
          if (Array.isArray(value))
            fileIsUploading = value.find(
              (item) => item.transferMethod === TransferMethod.local_file && !item.uploadedId,
            );
          else fileIsUploading = value.transferMethod === TransferMethod.local_file && !value.uploadedId;

          if (fileIsUploading) errMsg = t('appDebug.errorMessage.waitForFileUpload');
        }
      });
    });
    if (errMsg) {
