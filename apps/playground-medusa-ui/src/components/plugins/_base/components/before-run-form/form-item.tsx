...
import Select from '@plugins/_ui/select';
import Input from '@plugins/_ui/input';
import Textarea from '@plugins/_ui/textarea';
import TextGenerationImageUploader from '@plugins/_ui/image-uploader/text-generation-image-uploader';
import { FileUploaderInAttachmentWrapper } from '@plugins/_ui/file-uploader';
import { Resolution, TransferMethod } from '@plugins/_ui/image-uploader/types';
import { useFeatures } from '@plugins/_ui/features/hooks';
...
import { BubbleX } from '@plugins/_icons/src/vender/line/others';
import { FILE_EXTS } from '../../types';
...
const FormItem: FC<Props> = ({ payload, value, onChange, className, autoFocus, inStepRun = false }) => {
  ...
  const singleFileValue = useMemo(() => {
    if (payload.variable === '#files#') return value?.[0] || [];
  ...
  return (
    <div className={cn(className)}>
      {!isArrayLikeType && (
        <div className="system-sm-semibold text-text-secondary mb-1 flex h-6 items-center gap-1">
          <div className="truncate">{typeof payload.label === 'object' ? nodeKey : payload.label}</div>
          {!payload.required && (
            <span className="system-xs-regular text-text-tertiary">{t('workflow.panel.optional')}</span>
          )}
        </div>
      )}
      <div className="grow">
        ...
        {type === InputVarType.json && (
          <CodeEditor value={value} title={<span>JSON</span>} language={CodeLanguage.json} onChange={onChange} />
        )}
