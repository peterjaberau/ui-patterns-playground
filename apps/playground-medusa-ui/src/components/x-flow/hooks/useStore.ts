...
const useStore = <T = unknown>(selector: (state: FlowState) => T, equalityFn?: (a: T, b: T) => boolean) => {
...
const useStoreApi = <NodeType extends FlowNode = FlowNode, EdgeType extends Edge = Edge>() => {
  const store = useContext(StoreContext);
