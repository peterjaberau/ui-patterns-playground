...
>({
  ...
  const { data, error, fetchNextPage, fetchPreviousPage, hasPreviousPage, hasNextPage, isFetching, isPending } =
    useInfiniteQuery({
      queryKey: queryKey,
  ...
  return (
    <div ref={parentRef} className="h-full">
      {items?.length ? items.map((item) => <div key={item.id}>{renderItem(item)}</div>) : renderEmpty()}
