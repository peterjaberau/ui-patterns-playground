...
export const useWorkflowVersionHistory = (params: FetchWorkflowDraftPageParams) => {
  ...
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam = 1 }) =>
      get<FetchWorkflowDraftPageResponse>(`/apps/${appId}/workflows`, {
        params: {
          page: pageParam,
          limit,
          user_id: userId || '',
...
export const useUpdateWorkflow = (appId: string) => {
  return useMutation({
    ...
    mutationFn: (params: UpdateWorkflowParams) =>
      patch(`/apps/${appId}/workflows/${params.workflowId}`, {
        body: {
...
export const usePublishWorkflow = (appId: string) => {
  return useMutation({
    ...
    mutationFn: (params: PublishWorkflowParams) =>
      post<CommonResponse & { created_at: number }>(`/apps/${appId}/workflows/publish`, {
        body: {
          marked_name: params.title,
          marked_comment: params.releaseNotes,
        },
