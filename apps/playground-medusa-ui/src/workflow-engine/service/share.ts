...
import type { AppConversationData, AppData, AppMeta, ConversationItem } from '@workflow-app/models/share';
import type { ChatConfig } from '@base/chat/types';
import type { SystemFeatures } from '@workflow-app/types/feature';
...
export const fetchConversations = async (
  ...
  return getAction('get', isInstalledApp)(getUrl('conversations', isInstalledApp, installedAppId), {
    params: { ...{ limit: limit || 20 }, ...(last_id ? { last_id } : {}), ...(pinned !== undefined ? { pinned } : {}) },
...
export const renameConversation = async (isInstalledApp: boolean, installedAppId = '', id: string, name: string) => {
  return getAction('post', isInstalledApp)(getUrl(`conversations/${id}/name`, isInstalledApp, installedAppId), {
    body: { name },
  });
};
...
export const fetchMoreLikeThis = async (messageId: string, isInstalledApp: boolean, installedAppId = '') => {
  return getAction('get', isInstalledApp)(
    getUrl(`/messages/${messageId}/more-like-this`, isInstalledApp, installedAppId),
    {
      params: {
...
export const saveMessage = (messageId: string, isInstalledApp: boolean, installedAppId = '') => {
  return getAction('post', isInstalledApp)(getUrl('/saved-messages', isInstalledApp, installedAppId), {
...
export const fetchSuggestedQuestions = (messageId: string, isInstalledApp: boolean, installedAppId = '') => {
  return getAction(
    'get',
    isInstalledApp,
  )(getUrl(`/messages/${messageId}/suggested-questions`, isInstalledApp, installedAppId));
};

export const audioToText = (url: string, isPublicAPI: boolean, body: FormData) => {
  return getAction('post', !isPublicAPI)(url, { body }, { bodyStringify: false, deleteContentType: true }) as Promise<{
    text: string;
  }>;
};
...
export const textToAudioStream = (
  ...
  body: { streaming: boolean; voice?: string; message_id?: string; text?: string | null | undefined },
) => {
