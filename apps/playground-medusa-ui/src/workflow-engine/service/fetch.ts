...
import { API_PREFIX, MARKETPLACE_API_PREFIX, PUBLIC_API_PREFIX } from '@workflow-app/config';
      ...
      switch (clonedResponse.status) {
        case 403:
          bodyJson.then((data: ResponseError) => {
            if (!otherOptions.silent) Toast.notify({ type: 'error', message: data.message });
...
const beforeErrorToast = (otherOptions: IOtherOptions): BeforeErrorHook => {
  return (error) => {
    if (!otherOptions.silent) Toast.notify({ type: 'error', message: error.message });
    return error;
  };
};
...
async function base<T>(url: string, options: FetchOptionType = {}, otherOptions: IOtherOptions = {}): Promise<T> {
  ...
  const base = isMarketplaceAPI ? MARKETPLACE_API_PREFIX : isPublicAPI ? PUBLIC_API_PREFIX : API_PREFIX;
  ...
  const client = baseClient.extend({
    hooks: {
      ...baseHooks,
      beforeError: [...(baseHooks.beforeError || []), beforeErrorToast(otherOptions)],
      beforeRequest: [
  ...
  if (contentType && [ContentType.download, ContentType.audio, ContentType.downloadZip].includes(contentType))
