...
function CodeGroupPanels({ children, targetCode, ...props }: ICodeGroupPanelsProps) {
  ...
  return (
    <CodePanel {...props} targetCode={targetCode}>
      {children}
    </CodePanel>
  );
}
...
function useTabGroupProps(availableLanguages: string[]) {
  ...
  return {
    ...
    onChange: (newSelectedIndex: number) => {
      preventLayoutShift(() => addPreferredLanguage(availableLanguages[newSelectedIndex]) as any);
    },
  };
}
...
export function CodeGroup({ children, title, inputs, targetCode, ...props }: IChildrenProps) {
  const languages = Children.map(children, (child: any) => getPanelTitle(child.props.children.props));
  ...
  return (
    <CodeGroupContext.Provider value={true}>
      ...
      >
        ...
        <CodeGroupPanels {...props} targetCode={targetCode}>
          {children}
        </CodeGroupPanels>
      </Container>
...
export function Code({ children, ...props }: IChildProps) {
  ...
  if (isGrouped) return <code {...props} dangerouslySetInnerHTML={{ __html: children }} />;
