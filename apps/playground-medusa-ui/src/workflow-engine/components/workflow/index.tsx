...
const Workflow: FC<WorkflowProps> = memo(({ nodes: originalNodes, edges: originalEdges, viewport }) => {
  ...
  return (
    <div
      ...
      className={`relative h-full w-full min-w-[960px] ${workflowReadOnly && 'workflow-panel-animation'} ${nodeAnimation && 'workflow-node-animation'} `}
  ...
  const initialFeatures: FeaturesData = {
    file: {
      ...
      allowed_file_extensions:
        ...
        FILE_EXTS[SupportUploadFileTypes.image].map((ext: any) => `.${ext}`),
