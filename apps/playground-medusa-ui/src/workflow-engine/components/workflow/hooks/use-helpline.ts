  ...
  const handleSetHelpline = useCallback(
    (node: Node) => {
      ...
      if (showHorizontalHelpLineNodesLength > 0) {
        ...
        if (node.position.x > last.position.x) helpLine.width = node.position.x + node.width! - first.position.x;
      ...
      if (showVerticalHelpLineNodesLength > 0) {
        ...
        if (node.position.y > last.position.y) helpLine.height = node.position.y + node.height! - first.position.y;
