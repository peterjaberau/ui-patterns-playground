    ...
    return BLOCK_CLASSIFICATIONS.reduce(
      (acc, classification) => {
        const list = groupBy(blocks, 'classification')[classification].filter((block) => {
          ...
          return (
            block.title.toLowerCase().includes(searchText.toLowerCase()) && availableBlocksTypes.includes(block.type)
          );
        });
          ...
          {list.map((block) => (
            ...
            >
              ...
              >
                ...
                <div className="text-text-secondary grow text-sm">{block.title}</div>
                {block.type === BlockEnum.LoopEnd && (
                  <Badge text={t('workflow.nodes.loop.loopNode')} className="ml-2 shrink-0" />
                )}
              </div>
            </Tooltip>
          ))}
