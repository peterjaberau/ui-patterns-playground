...
const ToolItem: FC<Props> = ({ provider, payload, onSelect, disabled }) => {
  ...
  return (
    <Tooltip
      ...
      popupContent={
        <div>
          <BlockIcon size="md" className="mb-2" type={BlockEnum.Tool} toolIcon={provider.icon} />
    ...
    >
      ...
      >
        <div
          className={cn(
            'system-sm-medium border-divider-subtle text-text-secondary h-8 truncate border-l-2 pl-4 leading-8',
            disabled && 'opacity-30',
          )}
        >
        ...
        {disabled && (
          <Badge className="text-text-tertiary flex h-5 items-center space-x-0.5" uppercase>
