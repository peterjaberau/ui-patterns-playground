...
const ColorPicker = ({ theme, onThemeChange }: ColorPickerProps) => {
  ...
  return (
    <PortalToFollowElem open={open} onOpenChange={setOpen} placement="top" offset={4}>
      <PortalToFollowElemTrigger onClick={() => setOpen(!open)}>
        ...
        >
          <div className={cn('h-4 w-4 rounded-full border border-black/5', THEME_MAP[theme].title)}></div>
        </div>
