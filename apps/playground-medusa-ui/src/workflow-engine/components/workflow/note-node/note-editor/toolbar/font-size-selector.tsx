  ...
  return (
    ...
    >
      <PortalToFollowElemTrigger onClick={() => handleOpenFontSizeSelector(!fontSizeSelectorShow)}>
        <div
          className={cn(
            'text-text-tertiary hover:bg-state-base-hover hover:text-text-secondary flex h-8 cursor-pointer items-center rounded-md pl-2 pr-1.5 text-[13px] font-medium',
      ...
      <PortalToFollowElemContent>
        <div className="border-components-panel-border bg-components-panel-bg-blur text-text-secondary w-[120px] rounded-md border-[0.5px] p-1 shadow-xl">
          {FONT_SIZE_LIST.map((font) => (
            ...
            >
              ...
              {fontSize === font.key && <Check className="text-text-accent h-4 w-4" />}
            </div>
          ))}
        </div>
      </PortalToFollowElemContent>
