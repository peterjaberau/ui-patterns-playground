...
const NoteNode = ({ id, data }: NodeProps<NoteNodeType>) => {
  ...
  return (
    <div
      ...
      ref={ref}
    >
      <NoteEditorContextProvider key={controlPromptEditorRerenderKey} value={data.text}>
        <>
          <NodeResizer nodeId={id} nodeData={data} icon={<Icon />} minWidth={240} minHeight={88} />
          <div className={cn('h-2 shrink-0 rounded-t-md opacity-50', THEME_MAP[theme].title)}></div>
          {data.selected && (
          ...
          {data.showAuthor && <div className="text-text-tertiary p-3 pt-0 text-xs">{data.author}</div>}
        </>
