  ...
  return (
    <div
      className={cn(
        ...
        status === 'succeeded' &&
          'bg-workflow-display-success-bg text-text-success border-[rgba(23,178,106,0.8)] bg-[url(@workflow/run/assets/bg-line-success.svg)]',
        status === 'succeeded' &&
          theme === Theme.light &&
        ...
        status === 'partial-succeeded' &&
          ...
          'shadow-[inset_2px_2px_0_0_rgba(255,255,255,0.12),inset_0_1px_3px_0_rgba(0,0,0,0.4),inset_0_2px_24px_0_rgba(23,178,106,0.25),0_1px_2px_0_rgba(0,0,0,0.1),0_0_0_1px_rgba(24, 24, 27, 0.95)]',
        status === 'failed' &&
          'bg-workflow-display-error-bg text-text-warning border-[rgba(240,68,56,0.8)] bg-[url(@workflow/run/assets/bg-line-error.svg)]',
        status === 'failed' &&
          theme === Theme.light &&
        ...
        status === 'stopped' &&
          'bg-workflow-display-warning-bg text-text-destructive border-[rgba(247,144,9,0.8)] bg-[url(@workflow/run/assets/bg-line-warning.svg)]',
        status === 'stopped' &&
          theme === Theme.light &&
        ...
        status === 'exception' &&
          ...
          'shadow-[inset_2px_2px_0_0_rgba(255,255,255,0.12),inset_0_1px_3px_0_rgba(0,0,0,0.4),inset_0_2px_24px_0_rgba(247,144,9,0.25),0_1px_2px_0_rgba(0,0,0,0.1),0_0_0_1px_rgba(24, 24, 27, 0.95)]',
        status === 'running' &&
          'bg-workflow-display-normal-bg text-util-colors-blue-light-blue-light-600 border-[rgba(11,165,236,0.8)] bg-[url(@workflow/run/assets/bg-line-running.svg)]',
        status === 'running' &&
          theme === Theme.light &&
    ...
    >
      <div
        className={cn(
          ...
          theme === Theme.light && 'bg-[url(@workflow/run/assets/highlight.svg)]',
          theme === Theme.dark && 'bg-[url(@workflow/run/assets/highlight-dark.svg)]',
        )}
      ></div>
      {children}
    </div>
