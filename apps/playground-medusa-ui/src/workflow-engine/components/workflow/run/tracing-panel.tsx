...
const TracingPanel: FC<TracingPanelProps> = ({
  ...
  const handleParallelMouseLeave = useCallback((e: React.MouseEvent) => {
    ...
    if (relatedTarget && 'closest' in relatedTarget) {
      ...
      if (closestParallel) setHoveredParallel(closestParallel.getAttribute('data-parallel-id'));
      else setHoveredParallel(null);
    } else {
    ...
    if (isParallelFirstNode) {
          ...
          <div className="mb-1 flex items-center">
            <button
              ...
              className={cn(
                ...
                isHovered
                  ? 'border-components-button-primary-border bg-components-button-primary-bg text-text-primary-on-surface rounded'
            ...
            >
              {isHovered ? (
                ...
                <RiMenu4Line className="text-text-tertiary h-3 w-3" />
              )}
            </button>
            ...
            <div
              className="bg-divider-subtle mx-2 h-px grow"
      ...
      return (
        <div key={node.id}>
          <div
            className={cn(
              'system-2xs-medium-uppercase -mb-1.5 pl-4',
