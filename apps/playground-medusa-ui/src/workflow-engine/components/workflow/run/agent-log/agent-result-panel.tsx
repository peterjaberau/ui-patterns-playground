...
const AgentResultPanel = ({
  ...
  return (
    <div className="bg-background-section overflow-y-auto">
      <AgentLogNav agentOrToolLogItemStack={agentOrToolLogItemStack} onShowAgentOrToolLog={onShowAgentOrToolLog} />
      {
        <div className="space-y-1 p-2">
          {list.map((item) => (
            <AgentLogItem key={item.id} item={item} onShowAgentOrToolLog={onShowAgentOrToolLog} />
          ))}
        </div>
      }
      {top.hasCircle && (
