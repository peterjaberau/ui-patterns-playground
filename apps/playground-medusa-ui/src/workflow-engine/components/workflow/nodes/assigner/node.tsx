...
import { BlockEnum, type Node, type NodeProps } from '@workflow/types';
...
const NodeComponent: FC<NodeProps<AssignerNodeType>> = ({ data }) => {
  ...
  if (data.version === '2') {
    ...
    const validOperationItems =
      operationItems?.filter((item) => item.variable_selector && item.variable_selector.length > 0) || [];
        ...
        <div className="relative flex flex-col items-start gap-0.5 self-stretch px-3 py-1">
          <div className="flex flex-col items-start gap-1 self-stretch">
            <div className="bg-workflow-block-parma-bg flex items-center gap-1 self-stretch rounded-md px-[5px] py-1">
              <div className="system-xs-medium text-text-tertiary flex-1">{t(`${i18nPrefix}.varNotSet`)}</div>
            </div>
          </div>
        </div>
      );
