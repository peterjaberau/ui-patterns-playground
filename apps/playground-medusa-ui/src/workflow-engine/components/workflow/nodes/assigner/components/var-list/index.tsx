...
import ListNoDataPlaceholder from '@workflow/nodes/_base/components/list-no-data-placeholder';
import VarReferencePicker from '@workflow/nodes/_base/components/variable/var-reference-picker';
import type { ValueSelector, Var, VarType } from '@workflow/types';
...
const VarList: FC<Props> = ({
    ...
    (index: number) => {
      return (item: { value: string | number }) => {
        const newList = produce(list, (draft) => {
          ...
          else draft[index].input_type = AssignerNodeInputType.variable;
        });
        onChange(newList);
      };
    },
  ...
  const handleFilterToAssignedVar = useCallback(
    (index: number) => {
      return (payload: Var) => {
        ...
        if (item.variable_selector.join('.') === `${payload.nodeId}.${payload.variable}`) return false;

        if (!filterToAssignedVar || !item.variable_selector || !assignedVarType || !item.operation) return true;

        return filterToAssignedVar(payload, assignedVarType, item.operation);
      };
    },
  ...
  if (list.length === 0) {
    return <ListNoDataPlaceholder>{t('workflow.nodes.assigner.noVarTip')}</ListNoDataPlaceholder>;
  }
      ...
      {list.map((item, index) => {
        ...
        return (
          <div className="flex items-start gap-1 self-stretch" key={index}>
            ...
            <ActionButton
              size="l"
              className="hover:!bg-state-destructive-hover group shrink-0"
