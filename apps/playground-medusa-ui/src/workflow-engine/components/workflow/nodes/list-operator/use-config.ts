...
import { useIsChatMode, useNodesReadOnly, useWorkflow, useWorkflowVariables } from '@workflow/hooks';
...
const useConfig = (id: string, payload: ListFilterNodeType) => {
  ...
  const itemVarTypeShowName = useMemo(() => {
    if (!inputs.variable) return '?';
    return [
      (itemVarType || VarType.string).substring(0, 1).toUpperCase(),
      (itemVarType || VarType.string).substring(1),
    ].join('');
  ...
  const handleVarChanges = useCallback(
    (variable: ValueSelector | string) => {
      const newInputs = produce(inputs, (draft) => {
        ...
        if (isFileArray && draft.order_by.enabled && !draft.order_by.key) draft.order_by.key = 'name';
      });
      setInputs(newInputs);
    },
  ...
  const handleFilterEnabledChange = useCallback(
    (enabled: boolean) => {
      const newInputs = produce(inputs, (draft) => {
        ...
        if (enabled && !draft.filter_by.conditions) draft.filter_by.conditions = [];
      });
      setInputs(newInputs);
    },
  ...
  const handleExtractsEnabledChange = useCallback(
    (enabled: boolean) => {
      const newInputs = produce(inputs, (draft) => {
        draft.extract_by.enabled = enabled;
        if (enabled) draft.extract_by.serial = '1';
      });
      setInputs(newInputs);
    },
    [inputs, setInputs],
  ...
  const handleExtractsChange = useCallback(
    (value: string) => {
      const newInputs = produce(inputs, (draft) => {
  ...
  const handleOrderByEnabledChange = useCallback(
    (enabled: boolean) => {
      const newInputs = produce(inputs, (draft) => {
        draft.order_by.enabled = enabled;
        if (enabled) {
          draft.order_by.value = OrderBy.ASC;
          if (hasSubVariable && !draft.order_by.key) draft.order_by.key = 'name';
        }
      });
      setInputs(newInputs);
    },
  ...
  const handleOrderByKeyChange = useCallback(
    (key: string) => {
      const newInputs = produce(inputs, (draft) => {
  ...
  const handleOrderByTypeChange = useCallback(
    (type: OrderBy) => {
      return () => {
        const newInputs = produce(inputs, (draft) => {
