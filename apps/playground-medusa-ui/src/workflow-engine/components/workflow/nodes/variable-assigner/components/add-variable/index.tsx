...
const AddVariable = ({
  ...
  return (
    <div className={cn(open && '!flex', variableAssignerNodeData.selected && '!flex')}>
      <PortalToFollowElem placement={'right'} offset={4} open={open} onOpenChange={setOpen}>
        <PortalToFollowElemTrigger onClick={() => setOpen(!open)}>
          ...
          >
            <Plus02
              className={cn('h-2.5 w-2.5 text-gray-500', 'group-hover/addvariable:text-white', open && '!text-white')}
            />
          </div>
