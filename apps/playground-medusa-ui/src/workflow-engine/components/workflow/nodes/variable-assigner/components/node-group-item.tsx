...
const NodeGroupItem = ({ item }: NodeGroupItemProps) => {
  ...
  const showSelectionBorder = useMemo(() => {
    if (groupEnabled && enteringNodePayload?.nodeId === item.variableAssignerNodeId) {
      if (hoveringAssignVariableGroupId) return hoveringAssignVariableGroupId !== item.targetHandleId;
      else return enteringNodePayload?.nodeData.advanced_settings?.groups[0].groupId !== item.targetHandleId;
    }
  ...
  return (
    ...
    >
      <div className="flex h-4 items-center justify-between text-[10px] font-medium text-gray-500">
        <span className={cn('grow truncate uppercase', showSelectedBorder && 'text-primary-600')} title={item.title}>
          {item.title}
        </span>
