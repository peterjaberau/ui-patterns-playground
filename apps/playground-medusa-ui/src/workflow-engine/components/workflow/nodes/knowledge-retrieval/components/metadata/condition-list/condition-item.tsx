...
import { COMMON_VARIABLE_REGEX, VARIABLE_REGEX, comparisonOperatorNotRequireValue } from './utils';
...
import type {
  ...
  MetadataShape,
} from '@workflow/nodes/knowledge-retrieval/types';
...
const ConditionItem = ({
  ...
  return (
    <div className={cn('mb-1 flex last-of-type:mb-0', className)}>
      <div className={cn('bg-components-input-bg-normal grow rounded-lg', isHovered && 'bg-state-destructive-hover')}>
        <div className="flex items-center p-1">
          <div className="w-0 grow">
            <div className="border-components-panel-border-subtle bg-components-badge-white-to-dark shadow-xs inline-flex h-6 items-center rounded-md border-[0.5px] pl-1 pr-1.5">
              <div className="mr-0.5 p-[1px]">
                <MetadataIcon type={currentMetadata?.type} className="h-3 w-3" />
              </div>
              <div className="system-xs-medium text-text-secondary mr-0.5">{currentMetadata?.name}</div>
