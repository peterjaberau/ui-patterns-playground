...
import { DEFAULT_WEIGHTED_SCORE, RerankingModeEnum } from '@workflow-app/models/datasets';
...
export const checkoutRerankModelConfigedInRetrievalSettings = (
  ...
  const { allEconomic, allExternal } = getSelectedDatasetsMode(datasets);
  ...
  if (reranking_mode === RerankingModeEnum.RerankingModel && (!reranking_model?.provider || !reranking_model?.model)) {
    if ((allEconomic || allExternal) && !reranking_enable) return true;
