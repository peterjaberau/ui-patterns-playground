...
import { useIsChatMode, useIsNodeInLoop, useNodesReadOnly, useWorkflow } from '../../hooks';
...
const useConfig = (id: string, payload: LoopNodeType) => {
  ...
  const filterInputVar = useCallback((varPayload: Var) => {
    return [VarType.array, VarType.arrayString, VarType.arrayNumber, VarType.arrayObject, VarType.arrayFile].includes(
      varPayload.type,
    );
  }, []);
  ...
  const [isShowLoopDetail, { setTrue: doShowLoopDetail, setFalse: doHideLoopDetail }] = useBoolean(false);
  ...
  const { usedOutVars, allVarObject } = (() => {
    ...
    loopChildrenNodes.forEach((node) => {
      ...
      nodeVars.forEach((varSelector) => {
        if (varSelector[0] === id) {
          // skip Loop node itself variable: item, index
          return;
        }
        const isInLoop = isNodeInLoop(varSelector[0]);
        if (isInLoop)
          // not pass loop inner variable
          return;
  ...
  const changeErrorResponseMode = useCallback(
    (item: { value: unknown }) => {
  ...
  const handleAddCondition = useCallback<HandleAddCondition>(
    (valueSelector, varItem) => {
      const newInputs = produce(inputs, (draft) => {
        if (!draft.break_conditions) draft.break_conditions = [];
        ...
        draft.break_conditions?.push({
          ...
          comparison_operator: getOperators(
            varItem.type,
            getIsVarFileAttribute(valueSelector) ? { key: valueSelector.slice(-1)[0] } : undefined,
          )[0],
          value: '',
        });
      });
  ...
  const handleUpdateCondition = useCallback<HandleUpdateCondition>(
    (conditionId, newCondition) => {
      const newInputs = produce(inputs, (draft) => {
        ...
        if (targetCondition) Object.assign(targetCondition, newCondition);
      });
      setInputs(newInputs);
    },
    [inputs, setInputs],
  ...
  const handleToggleConditionLogicalOperator = useCallback<HandleToggleConditionLogicalOperator>(() => {
    const newInputs = produce(inputs, (draft) => {
      draft.logical_operator =
        draft.logical_operator === LogicalOperator.and ? LogicalOperator.or : LogicalOperator.and;
    });
    setInputs(newInputs);
  }, [inputs, setInputs]);

  const handleAddSubVariableCondition = useCallback<HandleAddSubVariableCondition>(
    (conditionId: string, key?: string) => {
      const newInputs = produce(inputs, (draft) => {
        ...
        if (subVarCondition) {
          ...
          subVarCondition.conditions.push({
            ...
            comparison_operator:
              svcComparisonOperators && svcComparisonOperators.length ? svcComparisonOperators[0] : undefined,
            value: '',
          });
        }
      });
  ...
  const handleRemoveSubVariableCondition = useCallback(
    (conditionId: string, subConditionId: string) => {
      const newInputs = produce(inputs, (draft) => {
        const condition = draft.break_conditions?.find((item) => item.id === conditionId);
        if (!condition) return;
  ...
  const handleUpdateSubVariableCondition = useCallback<HandleUpdateSubVariableCondition>(
    (conditionId, subConditionId, newSubCondition) => {
      const newInputs = produce(inputs, (draft) => {
        const targetCondition = draft.break_conditions?.find((item) => item.id === conditionId);
  ...
  const handleUpdateLoopCount = useCallback(
    (value: number) => {
      const newInputs = produce(inputs, (draft) => {
  ...
  const handleAddLoopVariable = useCallback(() => {
    const newInputs = produce(inputsRef.current, (draft) => {
      if (!draft.loop_variables) draft.loop_variables = [];
  ...
  const handleRemoveLoopVariable = useCallback(
    (id: string) => {
      const newInputs = produce(inputsRef.current, (draft) => {
  ...
  const handleUpdateLoopVariable = useCallback(
