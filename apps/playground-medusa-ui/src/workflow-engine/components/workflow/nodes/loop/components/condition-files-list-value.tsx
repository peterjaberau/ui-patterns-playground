...
const ConditionValue = ({ condition }: ConditionValueProps) => {
  ...
  const variableName = isSystemVar(variableSelector)
  ...
  const selectName = useCallback(
    (c: Condition) => {
      const isSelect =
        c.comparison_operator === ComparisonOperator.in || c.comparison_operator === ComparisonOperator.notIn;
      if (isSelect) {
  ...
  return (
    <div className="bg-workflow-block-parma-bg rounded-md">
      <div className="flex h-6 items-center px-1">
        ...
        <div
          className={cn('text-text-accent shrink-0 truncate text-xs font-medium', !notHasValue && 'max-w-[70px]')}
          title={variableName}
        >
          {variableName}
        </div>
          ...
          <div className="relative flex h-6 items-center space-x-1" key={c.id}>
            ...
            <div className="system-xs-medium text-text-primary">
              {isComparisonOperatorNeedTranslate(c.comparison_operator)
                ? t(`workflow.nodes.ifElse.comparisonOperator.${c.comparison_operator}`)
