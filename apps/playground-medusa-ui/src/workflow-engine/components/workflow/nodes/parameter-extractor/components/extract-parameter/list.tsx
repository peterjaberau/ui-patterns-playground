...
import type { MoreInfo } from '@workflow/types';
...
const List: FC<Props> = ({ list, onChange }) => {
  ...
  if (list.length === 0) {
    return <ListNoDataPlaceholder>{t(`${i18nPrefix}.extractParametersNotSet`)}</ListNoDataPlaceholder>;
  }
  return (
    <div className="space-y-1">
      {list.map((item, index) => (
        <Item key={index} payload={item} onDelete={handleItemDelete(index)} onEdit={handleItemEdit(index)} />
      ))}
      {isShowEditModal && (
        <EditParam
          type="edit"
