...
const AddVariablePopupWithPosition = ({ nodeId, nodeData }: AddVariablePopupWithPositionProps) => {
  ...
  const outputType = useMemo(() => {
    ...
    if (!groupEnabled) return showAssignVariablePopup.variableAssignerNodeData.output_type;

    const group = showAssignVariablePopup.variableAssignerNodeData.advanced_settings?.groups.find(
      (group) => group.groupId === showAssignVariablePopup.variableAssignerNodeHandleId,
    );
