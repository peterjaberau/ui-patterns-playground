...
export const SettingItem = memo(({ label, children, status, tooltip }: SettingItemProps) => {
  const indicator: ComponentProps<typeof Indicator>['color'] =
    status === 'error' ? 'red' : status === 'warning' ? 'yellow' : undefined;
  const needTooltip = ['error', 'warning'].includes(status as any);
  return (
    <div className="bg-workflow-block-parma-bg relative flex items-center justify-between space-x-1 rounded-md px-1.5 py-1 text-xs font-normal">
      <div
        className={classNames(
          'text-text-tertiary system-xs-medium-uppercase shrink-0 truncate',
      ...
      <Tooltip popupContent={tooltip} disabled={!needTooltip}>
        <div className="system-xs-medium text-text-secondary truncate text-right">{children}</div>
      </Tooltip>
