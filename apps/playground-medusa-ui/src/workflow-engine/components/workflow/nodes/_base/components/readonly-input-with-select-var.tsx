...
const ReadonlyInputWithSelectVar: FC<Props> = ({ nodeId, value, className }) => {
          ...
          <div className="inline-flex h-[16px] items-center rounded-[5px] bg-white px-1.5">
            {!isEnv && !isChatVar && (
              <div className="flex items-center">
                <div className="p-[1px]">
                  <VarBlockIcon className="!text-gray-900" type={node?.type || BlockEnum.Start} />
                </div>
                <div className="mx-0.5 max-w-[60px] truncate text-xs font-medium text-gray-700" title={node?.title}>
                  {node?.title}
                </div>
  ...
  return <div className={cn('break-all text-xs', className)}>{res}</div>;
};
