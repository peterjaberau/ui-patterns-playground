...
import { hasErrorHandleNode } from '@workflow/utils';
...
const NextStep = ({ selectedNode }: NextStepProps) => {
  ...
  const connectedEdges = getConnectedEdges([selectedNode] as Node[], edges).filter(
          ...
          branch: {
            ...
            name:
              data.type === BlockEnum.QuestionClassifier
                ? `${t('workflow.nodes.questionClassifiers.class')} ${index + 1}`
                : branch.name,
          },
          nextNodes,
        };
  ...
  return (
    <div className="flex py-1">
      <div className="border-divider-regular bg-background-default shadow-xs relative flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border-[0.5px]">
        <BlockIcon type={selectedNode!.data.type} toolIcon={toolIcon} />
      </div>
      <Line list={list.length ? list.map((item) => item.nextNodes.length + 1) : [1]} />
