...
import { Stop } from '@base/icons/src/vender/line/mediaAndDevices';
...
const NodeControl: FC<NodeControlProps> = ({ id, data }) => {
  ...
  return (
    ...
    >
      <div
        className="border-components-actionbar-border bg-components-actionbar-bg text-text-tertiary flex h-6 items-center rounded-lg border-[0.5px] px-0.5 shadow-md backdrop-blur-[5px]"
        ...
        {canRunBySingle(data.type) && (
          ...
          >
            {data._isSingleRun ? (
              ...
              <Tooltip popupContent={t('workflow.panel.runThisStep')} asChild={false}>
                <RiPlayLargeLine className="h-3 w-3" />
              </Tooltip>
            )}
          </div>
        )}
        <PanelOperator id={id} data={data} offset={0} onOpenChange={handleOpenChange} triggerClassName="!w-5 !h-5" />
      </div>
    </div>
  );
};
