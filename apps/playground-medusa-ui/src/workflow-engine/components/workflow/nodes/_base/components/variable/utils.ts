...
import { BlockEnum, InputVarType, VarType } from '@workflow/types';
import type { StartNodeType } from '@workflow/nodes/start/types';
import type {
...
import type { Field as StructField } from '@workflow/nodes/llm/types';
  ...
  switch (type) {
    ...
    case BlockEnum.LLM: {
      ...
      if (isChatModel) {
        ...
        if (payload.memory?.query_prompt_template) prompts.push(payload.memory.query_prompt_template);
      } else {
    ...
    case BlockEnum.HttpRequest: {
      ...
      res = matchNotSystemVars([
        ...
        payload.headers,
        payload.params,
        typeof payload.body.data === 'string' ? payload.body.data : payload.body.data.map((d) => d.value).join(''),
      ]);
      break;
    }
    case BlockEnum.Tool: {
    ...
    case BlockEnum.Agent: {
      ...
      Object.keys(payload.agent_parameters || {}).forEach((key) => {
        ...
        if (typeof value === 'string') valueSelectors.push(...matchNotSystemVars([value]));
      });
      res = valueSelectors;
      break;
    }
...
export const getNodeUsedVarPassToServerKey = (node: Node, valueSelector: ValueSelector): string | string[] => {
  ...
  switch (type) {
    case BlockEnum.LLM: {
      ...
      if (payload.context?.variable_selector.join('.') === valueSelector.join('.')) res.push('#context#');
...
export const updateNodeVars = (oldNode: Node, oldVarSelector: ValueSelector, newVarSelector: ValueSelector): Node => {
  const newNode = produce(oldNode, (draft: any) => {
    ...
    switch (type) {
      case BlockEnum.End: {
        ...
        if (payload.outputs) {
          payload.outputs = payload.outputs.map((output) => {
            if (output.value_selector.join('.') === oldVarSelector.join('.')) output.value_selector = newVarSelector;
            return output;
          });
        }
        break;
      ...
      case BlockEnum.IfElse: {
        ...
        if (payload.conditions) {
          payload.conditions = payload.conditions.map((c) => {
            if (c.variable_selector?.join('.') === oldVarSelector.join('.')) c.variable_selector = newVarSelector;
            return c;
          });
        }
        break;
      ...
      case BlockEnum.ParameterExtractor: {
        const payload = data as ParameterExtractorNodeType;
        if (payload.query.join('.') === oldVarSelector.join('.')) payload.query = newVarSelector;
        payload.instruction = replaceOldVarInText(payload.instruction, oldVarSelector, newVarSelector);
        break;
      }
      case BlockEnum.Iteration: {
      ...
      case BlockEnum.ListFilter: {
        ...
        if (payload.variable.join('.') === oldVarSelector.join('.')) payload.variable = newVarSelector;
        break;
      }
    }
  });
