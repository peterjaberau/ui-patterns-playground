...
const RetryOnNode = ({ data }: RetryOnNodeProps) => {
  ...
  if (!retry_config?.retry_enabled) return null;

  if (!showDefault && !data._retryIndex) return null;

  return (
    <div className="px-3">
      <div
        className={cn(
          'system-xs-medium-uppercase bg-workflow-block-parma-bg text-text-tertiary flex items-center justify-between rounded-md border-[0.5px] border-transparent px-[5px] py-1',
      ...
      >
        <div className="flex items-center">
          {showDefault && t('workflow.nodes.common.retry.retryTimes', { times: retry_config.max_retries })}
          {isRunning && (
            <>
