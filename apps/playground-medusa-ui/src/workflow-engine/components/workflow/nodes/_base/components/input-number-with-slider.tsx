...
const InputNumberWithSlider: FC<Props> = ({ value, defaultValue = 0, min, max, readonly, onChange }) => {
  ...
  return (
    <div className="flex h-8 items-center justify-between space-x-2">
      <input
        ...
        disabled={readonly}
      />
      <Slider className="grow" value={value} min={min} max={max} step={1} onChange={onChange} disabled={readonly} />
    </div>
  );
};
