...
import type { CommonNodeType, OnSelectBlock } from '@workflow/types';
...
const Add = ({ nodeId, nodeData, sourceHandle, isParallel, isFailBranch }: AddProps) => {
  ...
  const tip = useMemo(() => {
    if (isFailBranch) return t('workflow.common.addFailureBranch');

    if (isParallel) return t('workflow.common.addParallelNode');
  ...
  const renderTrigger = useCallback(
    (open: boolean) => {
      return (
        <div
          className={`bg-dropzone-bg hover:bg-dropzone-bg-hover border-divider-regular text-text-placeholder relative flex h-9 cursor-pointer items-center rounded-lg border border-dashed px-2 text-xs ${open && '!bg-components-dropzone-bg-alt'} ${nodesReadOnly && '!cursor-not-allowed'} `}
        >
