...
export const ToolIcon = memo(({ providerName }: ToolIconProps) => {
  ...
  return (
    <Tooltip triggerMethod="hover" popupContent={tooltip} disabled={!notSuccess}>
      <div
        className={classNames(
          'border-components-panel-border-subtle bg-background-default-dodge relative flex size-5 items-center justify-center rounded-[6px] border-[0.5px]',
        )}
        ref={containerRef}
      >
        {!iconFetchError ? (
          <img
            src={icon}
            alt="tool icon"
            className={classNames('size-3.5 h-full w-full object-cover', notSuccess && 'opacity-50')}
