...
import { ErrorHandleMode, InputVarType, type NodePanelProps } from '@workflow/types';
...
import formatTracing from '@workflow/run/utils/format-log';

import { useLogs } from '@workflow/run/hooks';
    ...
    <div className="pb-2 pt-2">
      <div className="space-y-4 px-4 pb-4">
        <Field
          ...
          operations={
            <div className="system-2xs-medium-uppercase border-divider-deep text-text-tertiary flex h-[18px] items-center rounded-[5px] border px-1 capitalize">
              Array
            </div>
          }
        >
      ...
      <div className="px-4 pb-2">
        <Field
          title={t(`${i18nPrefix}.parallelMode`)}
          tooltip={<div className="w-[230px]">{t(`${i18nPrefix}.parallelPanelDesc`)}</div>}
          inline
        >
      ...
      {inputs.is_parallel && (
        <div className="px-4 pb-2">
          <Field
            title={t(`${i18nPrefix}.MaxParallelismTitle`)}
            isSubTitle
            tooltip={<div className="w-[230px]">{t(`${i18nPrefix}.MaxParallelismDesc`)}</div>}
          >
      ...
      <div className="px-4 py-2">
        <Field title={t(`${i18nPrefix}.errorResponseMethod`)}>
          <Select
            items={responseMethod}
            defaultValue={inputs.error_handle_mode}
            onSelect={changeErrorResponseMode}
            allowSearch={false}
          />
        </Field>
      </div>
      ...
      {isShowSingleRun && (
        <BeforeRunForm
          ...
          onStop={handleStop}
          {...logsParams}
          result={<ResultPanel {...runResult} showSteps={false} nodeInfo={nodeInfo} {...logsParams} />}
        />
      )}
    </div>
  );
