...
const ConditionValue = ({ variableSelector, labelName, operator, value }: ConditionValueProps) => {
  ...
  return (
    <div className="bg-workflow-block-parma-bg flex h-6 items-center rounded-md px-1">
      {!isEnvVar && !isChatVar && (
        <Variable02 className={cn('text-text-accent mr-1 h-3.5 w-3.5 shrink-0', isException && 'text-text-warning')} />
      )}
      ...
      <div
        className={cn(
          'text-text-accent ml-0.5 shrink-[2] truncate text-xs font-medium',
      ...
      <div className="text-text-primary mx-1 shrink-0 text-xs font-medium" title={operatorName}>
        {operatorName}
      </div>
      {!notHasValue && (
        <div className="text-text-secondary shrink-[3] truncate text-xs" title={formatValue}>
