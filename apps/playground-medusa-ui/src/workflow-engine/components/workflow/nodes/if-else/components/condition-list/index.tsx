...
import type { Node, NodeOutPutVar, Var } from '@workflow/types';
...
const ConditionList = ({
  ...
  const doToggleConditionLogicalOperator = useCallback(() => {
    if (isSubVariable) onToggleSubVariableConditionLogicalOperator?.(caseId!, conditionId!);
  ...
  return (
    <div className={cn('relative', !isSubVariable && 'pl-[60px]')}>
      {conditions.length > 1 && (
        ...
        >
          <div className="border-divider-deep absolute bottom-4 left-[46px] top-4 w-2.5 rounded-l-[8px] border border-r-0"></div>
          <div className="bg-components-panel-bg absolute right-0 top-1/2 h-[29px] w-4 -translate-y-1/2"></div>
          <div
            className="border-components-button-secondary-border bg-components-button-secondary-bg text-text-accent-secondary shadow-xs absolute right-1 top-1/2 flex h-[21px] -translate-y-1/2 cursor-pointer select-none items-center rounded-md border-[0.5px] px-1 text-[10px] font-semibold"
      ...
      {caseItem.conditions.map((condition) => (
        <ConditionItem
          ...
          file={
            varsIsVarFileAttribute[condition.id] ? { key: (condition.variable_selector || []).slice(-1)[0] } : undefined
          }
