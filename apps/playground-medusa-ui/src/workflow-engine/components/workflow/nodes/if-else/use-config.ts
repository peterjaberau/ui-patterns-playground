...
import { useEdgesInteractions, useNodesReadOnly } from '@workflow/hooks';
...
const useConfig = (id: string, payload: IfElseNodeType) => {
  ...
  const handleRemoveCondition = useCallback<HandleRemoveCondition>(
    (caseId, conditionId) => {
      const newInputs = produce(inputs, (draft) => {
        const targetCase = draft.cases?.find((item) => item.case_id === caseId);
        if (targetCase) targetCase.conditions = targetCase.conditions.filter((item) => item.id !== conditionId);
      });
      setInputs(newInputs);
    },
    [inputs, setInputs],
