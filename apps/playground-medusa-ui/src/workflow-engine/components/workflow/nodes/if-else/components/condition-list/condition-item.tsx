...
const ConditionItem = ({
  ...
  const doUpdateCondition = useCallback(
    (newCondition: Condition) => {
      if (isSubVariableKey) onUpdateSubVariableCondition?.(caseId, conditionId, condition.id, newCondition);
  ...
  const isSubVariable =
    ...
    [ComparisonOperator.contains, ComparisonOperator.notContains, ComparisonOperator.allOf].includes(
  ...
  const handleUpdateConditionValue = useCallback(
    (value: string) => {
      if (value === condition.value || (isArrayValue && value === condition.value?.[0])) return;
      const newCondition = {
        ...condition,
              ...
              <Select
                ...
                renderTrigger={(item) =>
                  item ? (
                    ...
                    <div className="system-sm-regular text-components-input-text-placeholder text-left">
