  ...
  useEventListener('click', (e) => {
    ...
    if (candidateNode) {
      ...
      const newNodes = produce(nodes, (draft: any) => {
        ...
        if (candidateNode.data.type === BlockEnum.Iteration) draft.push(getIterationStartNode(candidateNode.id));

        if (candidateNode.data.type === BlockEnum.Loop) draft.push(getLoopStartNode(candidateNode.id));
      });
      setNodes(newNodes);
      if (candidateNode.type === CUSTOM_NOTE_NODE) saveStateToHistory(WorkflowHistoryEvent.NoteAdd);
