...
const VariableTypeSelector = ({ inCell = false, value, list, onSelect, popupClassName }: Props) => {
  const [open, setOpen] = useState(false);

  return (
    <PortalToFollowElem open={open} onOpenChange={() => setOpen((v) => !v)} placement="bottom">
      <PortalToFollowElemTrigger className="w-full" onClick={() => setOpen((v) => !v)}>
        <div
          className={cn(
            'flex w-full cursor-pointer items-center px-2',
            !inCell && 'radius-md bg-components-input-bg-normal hover:bg-state-base-hover-alt py-1',
      ...
      <PortalToFollowElemContent className={cn('z-[11] w-full', popupClassName)}>
        <div className="radius-xl border-components-panel-border bg-components-panel-bg-blur border-[0.5px] p-1 shadow-lg">
          {list.map((item: any) => (
            <div
              key={item}
              className="radius-md hover:bg-state-base-hover flex cursor-pointer items-center gap-2 py-[6px] pl-3 pr-2"
              onClick={() => {
                onSelect(item);
                setOpen(false);
              }}
