...
const MenuItem: FC<MenuItemProps> = ({ item, onClick, isDestructive = false }) => {
  return (
    <div
      className={cn(
    ...
    >
      <div className={cn('system-md-regular text-text-primary flex-1', isDestructive && 'hover:text-text-destructive')}>
        {item.name}
      </div>
    </div>
  );
