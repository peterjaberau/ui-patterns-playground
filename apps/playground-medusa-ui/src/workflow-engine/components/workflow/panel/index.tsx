...
const Panel: FC = () => {
  ...
  const { currentLogItem, setCurrentLogItem, showMessageLogModal, setShowMessageLogModal, currentLogModalActiveTab } =
    useAppStore(
      useShallow((state) => ({
  ...
  return (
    <div tabIndex={-1} className={cn('absolute bottom-2 right-0 top-14 z-10 flex outline-none')} key={`${isRestoring}`}>
      {showMessageLogModal && (
        <MessageLogModal
          fixedWidth
          width={400}
