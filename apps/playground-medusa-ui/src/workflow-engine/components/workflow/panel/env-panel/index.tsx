...
const EnvPanel = () => {
  ...
  const handleSave = useCallback(
    async (env: EnvironmentVariable) => {
      ...
      if (!currentVar) {
        ...
        updateEnvList(
          newList.map((e) => (e.id === env.id && env.value_type === 'secret' ? { ...e, value: '[__HIDDEN__]' } : e)),
        );
        return;
  ...
  return (
    ...
    >
      ...
      <div className="system-sm-regular text-text-tertiary shrink-0 px-4 py-1">{t('workflow.env.envDescription')}</div>
