...
const DebugAndPreview = () => {
  ...
  const resize = useCallback(
    (e: MouseEvent) => {
      if (isResizing) {
        ...
        if (newWidth > 420 && newWidth < 1024) setPanelWidth(newWidth);
      }
    },
    [isResizing],
  );
  ...
  return (
    <div
      className={cn(
        'border-components-panel-border bg-chatbot-bg relative flex h-full flex-col rounded-l-2xl border border-r-0 shadow-xl',
      )}
    ...
    >
      ...
      />
      <div className="system-xl-semibold text-text-primary flex shrink-0 items-center justify-between px-4 pb-2 pt-3">
        ...
        <div className="flex items-center gap-1">
          ...
          {variables.length > 0 && (
            <div className="relative">
              ...
              {expanded && (
                <div className="border-components-panel-border-subtle bg-components-panel-on-panel-item-bg absolute bottom-[-17px] right-[5px] z-10 h-3 w-3 rotate-45 border-l-[0.5px] border-t-[0.5px]" />
              )}
            </div>
          )}
