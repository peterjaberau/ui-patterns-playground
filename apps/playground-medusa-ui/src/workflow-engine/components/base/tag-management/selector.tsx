...
const Panel = (props: PanelProps) => {
  ...
  const selectTag = (tag: Tag) => {
    if (selectedTagIDs.includes(tag.id)) setSelectedTagIDs(selectedTagIDs.filter((v) => v !== tag.id));
  ...
  return (
    <div className="border-components-panel-border bg-components-input-bg-hover relative w-full rounded-lg border-[0.5px]">
      ...
      {keywords && notExisted && (
        <div className="p-1">
          ...
          >
            ...
            <div className="text-text-secondary grow truncate text-sm leading-5">
              {'Create'}
      ...
      {(filteredTagList.length > 0 || filteredSelectedTagList.length > 0) && (
        <div className="max-h-[172px] overflow-y-auto p-1">
          {filteredSelectedTagList.map((tag) => (
            ...
            >
              <Checkbox className="shrink-0" checked={selectedTagIDs.includes(tag.id)} onCheck={() => {}} />
              <div title={tag.name} className="text-text-secondary grow truncate text-sm leading-5">
                {tag.name}
              </div>
            </div>
          ))}
      ...
      <div className="p-1">
        ...
        >
          <Tag03 className="text-text-tertiary h-4 w-4" />
          <div className="text-text-secondary grow truncate text-sm leading-5">{'Manage tabs'}</div>
        </div>
      </div>
    </div>
  );
...
const TagSelector: FC<TagSelectorProps> = ({
  ...
  const triggerContent = useMemo(() => {
    if (selectedTags?.length)
      return selectedTags
        .filter((selectedTag) => tagList.find((tag) => tag.id === selectedTag.id))
        .map((tag) => tag.name)
        .join(', ');
    return '';
  ...
  const Trigger = () => {
    return (
      <div
        className={cn(
          'group/tip hover:bg-state-base-hover relative flex w-full cursor-pointer items-center gap-1 rounded-md px-2 py-[7px]',
        )}
      >
        <Tag01 className="text-components-input-text-placeholder h-3 w-3 shrink-0" />
        <div className="system-sm-regular text-components-input-text-placeholder grow truncate text-start">
          {!triggerContent ? 'Add tag' : triggerContent}
        </div>
      </div>
    );
  };
