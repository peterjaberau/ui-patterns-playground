...
import ViewFormDropdown from '@base/chat/embedded-chatbot/inputs-form/view-form-dropdown';
...
const Header: FC<IHeaderProps> = ({ isMobile, customerIcon, title, theme, onCreateNewChat }) => {
  const { appData, currentConversationId, inputsForms } = useEmbeddedChatbotContext();
  if (!isMobile) {
    return (
  ...
  return (
    ...
    >
      <div className="flex grow items-center space-x-3">
        {customerIcon}
        <div className="system-md-semibold truncate" style={CssTransform(theme?.colorFontOnHeaderStyle ?? '')}>
          {title}
        </div>
      </div>
      <div className="flex items-center gap-1">
        ...
        {currentConversationId && inputsForms.length > 0 && <ViewFormDropdown iconColor={theme?.colorPathOnHeader} />}
      </div>
    </div>
  );
};
