...
import NewAudioButton from '@base/new-audio-button';
...
const Operation: FC<OperationProps> = ({
  ...
  const { id, isOpeningStatement, content: messageContent, annotation, feedback, adminFeedback, agent_thoughts } = item;
  ...
  const operationWidth = useMemo(() => {
    ...
    if (!isOpeningStatement && config?.supportAnnotation && config?.annotation_reply?.enabled) width += 26;
    if (config?.supportFeedback && !localFeedback?.rating && onFeedback && !isOpeningStatement) width += 60 + 8;
    if (config?.supportFeedback && localFeedback?.rating && onFeedback && !isOpeningStatement) width += 28 + 8;
    return width;
  }, [
    isOpeningStatement,
    showPromptLog,
  ...
  return (
    <>
      ...
      >
        ...
        {!isOpeningStatement && (
          <div className="border-components-actionbar-border bg-components-actionbar-bg ml-1 hidden items-center gap-0.5 rounded-[10px] border-[0.5px] p-0.5 shadow-md backdrop-blur-sm group-hover:flex">
            {config?.text_to_speech?.enabled && (
        ...
        {!isOpeningStatement && config?.supportFeedback && localFeedback?.rating && onFeedback && (
          <div className="border-components-actionbar-border bg-components-actionbar-bg ml-1 flex items-center gap-0.5 rounded-[10px] border-[0.5px] p-0.5 shadow-md backdrop-blur-sm">
