        ...
        >
          ...
          <div ref={chatContainerInnerRef} className={cn('w-full', !noSpacing && 'px-8', chatContainerInnerClassName)}>
            {chatList.map((item, index) => {
              ...
              return <Question key={item.id} item={item} questionIcon={questionIcon} theme={themeBuilder?.theme} />;
            })}
          </div>
        </div>
        <div
          className={`bg-chat-input-mask absolute bottom-0 flex justify-center ${(hasTryToAsk || !noChatInput || !noStopResponding) && chatFooterClassName}`}
          ref={chatFooterRef}
        >
          <div ref={chatFooterInnerRef} className={cn('relative', chatFooterInnerClassName)}>
            ...
            {hasTryToAsk && <TryToAsk suggestedQuestions={suggestedQuestions} onSend={onSend} isMobile={isMobile} />}
            {!noChatInput && (
              <ChatInputArea
