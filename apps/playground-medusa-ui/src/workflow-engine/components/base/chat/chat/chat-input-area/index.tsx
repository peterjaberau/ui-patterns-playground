...
// import { FileContextProvider, useFileStore } from '@base/file-uploader/store';
...
const ChatInputArea = ({
  ...
  return (
    <>
      <div
        className={cn(
          'border-components-chat-input-border bg-components-panel-bg-blur relative z-10 rounded-xl border pb-[9px] shadow-md',
          // isDragActive && 'border-components-option-card-option-selected-border border border-dashed',
          disabled && 'border-components-panel-border pointer-events-none opacity-50 shadow-none',
        )}
      >
        <div className="relative max-h-[158px] overflow-y-auto overflow-x-hidden px-[9px] pt-[9px]">
          <div>file chat input</div>
          ...
          {/* {showVoiceInput && ( */}
          {/*   <VoiceInput onCancel={() => setShowVoiceInput(false)} onConverted={(text) => setQuery(text)} /> */}
          {/* )} */}
        </div>
