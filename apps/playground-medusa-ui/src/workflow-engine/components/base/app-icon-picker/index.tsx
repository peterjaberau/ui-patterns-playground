...
import type { AppIconType, ImageFile } from '@workflow-app/types/app';
import cn from '@utils/classnames';
import { DISABLE_UPLOAD_IMAGE_AS_ICON } from '@workflow-app/config';
...
const AppIconPicker: FC<AppIconPickerProps> = ({ onSelect, onClose, className }) => {
  ...
  return (
    ...
    >
        ...
        <div className="w-full p-2 pb-0">
          <div className="bg-background-body flex items-center justify-center gap-2 rounded-xl p-1">
            {tabs.map((tab) => (
              <button
                key={tab.key}
                className={`flex h-8 flex-1 shrink-0 items-center justify-center rounded-xl p-2 text-sm font-medium ${activeTab === tab.key && 'bg-components-main-nav-nav-button-bg-active shadow-md'} `}
      ...
      <EmojiPickerInner
        className={cn(activeTab === 'emoji' ? 'block' : 'hidden', 'pt-2')}
