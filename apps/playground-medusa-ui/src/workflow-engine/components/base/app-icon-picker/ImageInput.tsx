...
import { ALLOW_FILE_EXTENSIONS } from '@workflow-app/types/app';
...
const ImageInput: FC<UploaderProps> = ({ className, cropShape, onImageInput }) => {
  ...
  const { isDragActive, handleDragEnter, handleDragOver, handleDragLeave, handleDrop } = useDraggableUploader(
    (file: File) => setInputImage({ file, url: URL.createObjectURL(file) }),
  );
    ...
    <div className={classNames(className, 'w-full px-3 py-1.5')}>
      <div
        className={classNames(
          ...
          'relative flex aspect-square flex-col items-center justify-center rounded-lg border-[1.5px] border-dashed border-gray-200 bg-gray-50 text-gray-500',
        )}
        ...
        {!inputImage ? (
          <>
            ...
            <div className="mb-[2px] text-sm font-medium">
              ...
              <button className="text-components-button-primary-bg" onClick={() => inputRef.current?.click()}>
                {t('common.imageInput.browse')}
              </button>
              <input
                ref={inputRef}
                type="file"
