    ...
    <>
      {log.length === 1 && (
        <div className="px-4 py-2">
          <div className="text-text-secondary whitespace-pre-line">{log[0].text}</div>
        </div>
      )}
      {log.length > 1 && (
        <div>
          {log.map((item, index) => (
            <div
              key={index}
              className="group/card hover:bg-state-base-hover mb-2 rounded-xl px-4 pb-4 pt-2 last-of-type:mb-0"
            >
