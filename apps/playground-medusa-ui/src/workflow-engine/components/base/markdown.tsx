...
import CopyIcon from '@base/copy-icon';
...
// import ThinkBlock from '@base/markdown-blocks/think-block';
...
const VideoBlock: any = memo(({ node }: any) => {
  ...
  return <div key={srcs.join()}>video gallery</div>;
});
...
const AudioBlock: any = memo(({ node }: any) => {
  ...
  return <div key={srcs.join()}>audio gallery</div>;
});
...
const Link = ({ node, ...props }: any) => {
  if (node.properties?.href && node.properties.href?.toString().startsWith('abbr')) {
    ...
    return (
      <abbr
        className="!decoration-primary-700 cursor-pointer underline decoration-dashed"
    ...
    return (
      <a {...props} target="_blank" className="!decoration-primary-700 cursor-pointer underline decoration-dashed">
...
export function Markdown(props: { content: string; className?: string; customDisallowedElements?: string[] }) {
  const latexContent = flow([preprocessThinkTag, preprocessLaTeX])(props.content);
