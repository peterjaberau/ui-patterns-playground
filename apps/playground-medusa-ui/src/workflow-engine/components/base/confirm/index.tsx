...
function Confirm({
  ...
  const confirmTxt = confirmText || 'Confirm';
  const cancelTxt = cancelText || 'Cancel';
  ...
  const handleClickOutside = (event: MouseEvent) => {
    if (maskClosable && dialogRef.current && !dialogRef.current.contains(event.target as Node)) onCancel();
  };
  ...
  return createPortal(
    <div
      className={'bg-background-overlay fixed inset-0 z-[10000000] flex items-center justify-center'}
      onClick={(e) => {
        e.preventDefault();
        e.stopPropagation();
      }}
    >
      <div ref={dialogRef} className={'relative w-full max-w-[480px] overflow-hidden'}>
        <div className="shadows-shadow-lg border-components-panel-border bg-components-panel-bg flex max-w-full flex-col items-start rounded-2xl border-[0.5px] border-solid">
