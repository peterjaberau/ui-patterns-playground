...
const ToolCallItem: FC<Props> = ({ toolCall, isLLM = false, isFinal, tokens, observation, finalAnswer }) => {
  ...
  return (
    <div className={cn('py-1')}>
      <div
        className={cn(
          'border-components-panel-border bg-background-default shadow-xs group rounded-2xl border transition-all hover:shadow-md',
        )}
      >
        <div
          className={cn('flex cursor-pointer items-center py-3 pl-[6px] pr-3', !collapseState && '!pb-2')}
        ...
        >
          ...
          />
          <div
            className={cn('text-text-secondary grow truncate text-[13px] font-semibold leading-[16px]')}
            title={toolName}
          >
            {toolName}
          </div>
          ...
          {toolCall.status === 'error' && <RiErrorWarningLine className="ml-2 h-3.5 w-3.5 shrink-0 text-[#F04438]" />}
        </div>
        {!collapseState && (
          <div className="pb-2">
