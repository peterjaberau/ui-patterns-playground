...
import { ALLOW_FILE_EXTENSIONS, TransferMethod } from '@workflow-app/types/app';
  ...
  const handleLocalFileUpload = useCallback(
    (file: File) => {
      ...
      reader.addEventListener(
        'error',
        () => {
          notify({ type: 'error', message: 'File reading failed, please try again.' });
        },
        false,
      );
...
export const useClipboardUploader = ({ visionConfig, onUpload, files }: useClipboardUploaderProps) => {
  ...
  const disabled = useMemo(
    () => !visionConfig || !visionConfig?.enabled || !allowLocalUpload || files.length >= visionConfig.number_limits!,
